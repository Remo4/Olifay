...
Server_DevOlifay = io.popen("echo $SSH_CLIENT | awk '{ print $1}'"):read('*a')
------------------------------------------------------------------------------------------------------------
local function Load_File()
local f = io.open("./Info_Sudo.lua", "r")  
if not f then   
if not redis:get(Server_DevOlifay.."Token_DevOlifay") then
...
redis:set(Server_DevOlifay.."Token_DevOlifay",token)
end 
else
io.write('\n\27[1;31m🔄┇Token was not saved \n لم يتم حفظ التوكن \n\27[0;39;49m')
end 
os.execute('lua Olifay.lua')
end
------------------------------------------------------------------------------------------------------------
if not redis:get(Server_DevOlifay.."User_DevOlifay") then
...
redis:set(Server_DevOlifay.."User_DevOlifay",User_Info.Info.Username)
redis:set(Server_DevOlifay.."Id_DevOlifay",User_Info.Info.Id)
else
...
------------------------------------------------------------------------------------------------------------
local Run_File_Olifay_Sudo = io.open("Info_Sudo.lua", 'w')
Run_File_Olifay_Sudo:write([[
do 
local Olifay_INFO = {
Id_DevOlifay = ]]..redis:get(Server_DevOlifay.."Id_DevOlifay")..[[,
UserName_Olifay = "]]..redis:get(Server_DevOlifay.."User_DevOlifay")..[[",
Token_Bot = "]]..redis:get(Server_DevOlifay.."Token_DevOlifay")..[["
}
return Olifay_INFO
end
]])
Run_File_Olifay_Sudo:close()
------------------------------------------------------------------------------------------------------------
local Run_File_Olifay = io.open("Olifay", 'w')
Run_File_Olifay:write([[
#!/usr/bin/env bash
THIS_DIR=$(cd $(dirname $0); pwd)
cd $THIS_DIR
token="]]..redis:get(Server_DevOlifay.."Token_DevOlifay")..[["
while(true) do
rm -fr ../.telegram-cli
./tg -s ./Olifay.lua -p PROFILE --bot=$token
done
]])
Run_File_Olifay:close()
------------------------------------------------------------------------------------------------------------
...
while(true) do
rm -fr ../.telegram-cli
screen -S Olifay -X kill
screen -S Olifay ./Olifay
done
]])
Run_SM:close()
...
os.execute('chmod +x Olifay')
os.execute('./oy')
Status = true
else   
f:close()  
redis:del(Server_DevOlifay.."Token_DevOlifay");redis:del(Server_DevOlifay.."Id_DevOlifay");redis:del(Server_DevOlifay.."User_DevOlifay")
Status = false
end  
return Status
end
...
UserName_Dev = sudos.UserName_Olifay
...
Status = true
elseif Dev_Olifay_User(user_id) then
Status = true  
elseif tonumber(user_id) == tonumber(bot_id) then  
Status = true  
elseif redis:sismember(bot_id.."Olifay:Sudo:User", user_id) then
Status = true  
elseif redis:sismember(bot_id.."Olifay:Basic:Constructor"..chat_id, user_id) then
Status = true
elseif redis:sismember(bot_id..'Olifay:Constructor'..chat_id, user_id) then
Status = true  
elseif redis:sismember(bot_id..'Olifay:Manager'..chat_id, user_id) then
Status = true  
elseif redis:sismember(bot_id..'Olifay:Mod:User'..chat_id, user_id) then
Status = true  
elseif redis:sismember(bot_id..'Olifay:Special:User'..chat_id, user_id) then  
Status = true  
else  
Status = false  
end  
...
if tonumber(user_id) == tonumber(721974232) then  
Status = "مطور السورس"  
elseif tonumber(user_id) == tonumber(1266770351) then  
Status = "مطور السورس"  
elseif Dev_Olifay_User(user_id) == true then
Status = "المطور الاساسي"  
elseif tonumber(user_id) == tonumber(bot_id) then  
Status = "البوت"
elseif redis:sismember(bot_id.."Olifay:Sudo:User", user_id) then
Status = redis:get(bot_id.."Olifay:Sudo:User:Reply"..chat_id) or "المطور"  
elseif redis:sismember(bot_id.."Olifay:Basic:Constructor"..chat_id, user_id) then
Status = redis:get(bot_id.."Olifay:Basic:Constructor:Reply"..chat_id) or "المنشئ اساسي"
elseif redis:sismember(bot_id..'Olifay:Constructor'..chat_id, user_id) then
Status = redis:get(bot_id.."Olifay:Constructor:Reply"..chat_id) or "المنشئ"  
elseif redis:sismember(bot_id..'Olifay:Manager'..chat_id, user_id) then
Status = redis:get(bot_id.."Olifay:Manager:Reply"..chat_id) or "المدير"  
elseif redis:sismember(bot_id..'Olifay:Mod:User'..chat_id, user_id) then
Status = redis:get(bot_id.."Olifay:Mod:User:Reply"..chat_id) or "الادمن"  
elseif redis:sismember(bot_id..'Olifay:Special:User'..chat_id, user_id) then  
Status = redis:get(bot_id.."Olifay:Special:User:Reply"..chat_id) or "المميز"  
else  
Status = redis:get(bot_id.."Olifay:Mempar:Group:Reply"..chat_id) or "العضو"
end  
return Status
end 
------------------------------------------------------------------------------------------------------------
function ChekBotAdd(chat_id)
if redis:sismember(bot_id.."Olifay:Chek:Groups",chat_id) then
Status = true
else 
Status = false
end
...
if redis:sismember(bot_id.."Olifay:Silence:User:Group"..Chat_id,User_id) then
Status = true
else
Status = false
end
...
if redis:sismember(bot_id.."Olifay:Removal:User:Group"..Chat_id,User_id) then
Status = true
else
Status = false
end
...
if redis:sismember(bot_id.."Olifay:Removal:User:Groups",User_id) then
Status = true
else
Status = false
end
...
redis:sadd(bot_id.."Olifay:Silence:User:Group"..msg.chat_id_,msg.sender_user_id_) 
...
------------------------------------------------------------------------------------------------------------
function FilesOlifayBot(msg)
for v in io.popen('ls Files'):lines() do
if v:match(".lua$") then
Text_FileBot = dofile("Files/"..v)
if Text_FileBot.OlifayBot and msg then
Text_FileBot = Text_FileBot.OlifayBot(msg)
end
end
end
...
local Get_Info = io.open('./'..bot_id..'Olifay:.json', "r"):read('*a')
local JsonInfo = JSON.decode(Get_Info)
var(JsonInfo)  
for Id_Group,Info_Group in pairs(JsonInfo.Groups) do
redis:set(bot_id.."Olifay:Lock:tagservrbot"..Id_Group,true)   
list ={"Lock:Bot:kick","Lock:User:Name","Lock:hashtak","Lock:Cmd","Lock:Link","Lock:forward","Lock:Keyboard","Lock:geam","Lock:Photo","Lock:Animation","Lock:Video","Lock:Audio","Lock:vico","Lock:Sticker","Lock:Document","Lock:Unsupported","Lock:Markdaun","Lock:Contact","Lock:Spam"}
for i,v in pairs(list) do
redis:set(bot_id..v..Id_Group,"del")
end
redis:sadd(bot_id.."Olifay:Chek:Groups",Id_Group)
if Info_Group.President then
for k,Id_President in pairs(Info_Group.President) do
redis:sadd(bot_id.."Olifay:Basic:Constructor"..Id_Group,Id_President)
end
end
if Info_Group.Constructor then
for k,Id_Constructor in pairs(Info_Group.Constructor) do
redis:sadd(bot_id.."Olifay:Constructor"..Id_Group,Id_Constructor)  
end
end
if Info_Group.Manager then
for k,Id_Manager in pairs(Info_Group.Manager) do
redis:sadd(bot_id.."Olifay:Manager"..Id_Group,Id_Manager)  
end
end
if Info_Group.Admin then
for k,Id_Admin in pairs(Info_Group.Admin) do
redis:sadd(bot_id.."Olifay:Mod:User"..Id_Group,Id_Admin)  
end
end
if Info_Group.Vips then
for k,Id_Vips in pairs(Info_Group.Vips) do
redis:sadd(bot_id.."Olifay:Special:User"..Id_Group,Id_Vips)  
end
end
if Info_Group.WelcomeGroup then
if Info_Group.WelcomeGroup ~= "" then
redis:set(bot_id.."Olifay:Get:Welcome:Group"..Id_Group,Info_Group.WelcomeGroup)   
end
end
if Info_Group.Status_Dev then
if Info_Group.Status_Dev ~= "" then
redis:set(bot_id.."Olifay:Sudo:User:Reply"..Id_Group,Info_Group.Status_Dev)   
end
end
if Info_Group.Status_Prt then
if Info_Group.Status_Prt ~= "" then
redis:set(bot_id.."Olifay:Basic:Constructor:Reply"..Id_Group,Info_Group.Status_Prt)   
end
end
if Info_Group.Status_Cto then
if Info_Group.Status_Cto ~= "" then
redis:set(bot_id.."Olifay:Constructor:Reply"..Id_Group,Info_Group.Status_Cto)   
end
end
if Info_Group.Status_Own then
if Info_Group.Status_Own ~= "" then
redis:set(bot_id.."Olifay:Manager:Reply"..Id_Group,Info_Group.Status_Own)   
end
end
if Info_Group.Status_Md then
if Info_Group.Status_Md ~= "" then
redis:set(bot_id.."Olifay:Mod:User:Reply"..Id_Group,Info_Group.Status_Md)   
end
end
if Info_Group.Status_Vip then
if Info_Group.Status_Vip ~= "" then
redis:set(bot_id.."Olifay:Special:User:Reply"..Id_Group,Info_Group.Status_Vip)   
end
end
if Info_Group.Status_Mem then
if Info_Group.Status_Mem ~= "" then
redis:set(bot_id.."Olifay:Mempar:Group:Reply"..Id_Group,Info_Group.Status_Mem)   
end
end
if Info_Group.LinkGroup then
if Info_Group.LinkGroup ~= "" then
redis:set(bot_id.."Olifay:link:set:Group"..Id_Group,Info_Group.LinkGroup)   
end
end
end
...
------------------------------------------------------------------------------------------------------------
function Dev_Olifay_File(msg,data)
if msg then
msg = data.message_
...
redis:incr(bot_id..'Olifay:Num:Message:User'..msg.chat_id_..':'..msg.sender_user_id_) 
TypeForChat = 'ForSuppur' 
elseif id:match("^(%d+)") then
redis:sadd(bot_id..'Olifay:UsersBot',msg.sender_user_id_)  
TypeForChat = 'ForUser' 
else
...
------------------------------------------------------------------------------------------------------------
if redis:get(bot_id.."Olifay:Lock:text"..msg.chat_id_) and not Vips(msg) then       
Delete_Message(msg.chat_id_,{[0] = msg.id_})   
return false     
end     
--------------------------------------------------------------------------------------------------------------
if msg.content_.ID == "MessageChatAddMembers" then 
redis:incr(bot_id.."Olifay:Num:Add:Memp"..msg.chat_id_..":"..msg.sender_user_id_) 
end
if msg.content_.ID == "MessageChatAddMembers" and not Vips(msg) then   
if redis:get(bot_id.."Olifay:Lock:AddMempar"..msg.chat_id_) == "kick" then
...
if redis:get(bot_id.."Olifay:Lock:Join"..msg.chat_id_) == "kick" then
KickGroup(msg.chat_id_,msg.sender_user_id_)
return false  
end
end
...
if redis:get(bot_id.."Olifay:Lock:User:Name"..msg.chat_id_) == "del" and not Vips(msg) then    
Delete_Message(msg.chat_id_,{[0] = msg.id_}) 
elseif redis:get(bot_id.."Olifay:Lock:User:Name"..msg.chat_id_) == "ked" and not Vips(msg) then    
RestrictChat(msg.chat_id_,msg.sender_user_id_)
Delete_Message(msg.chat_id_,{[0] = msg.id_}) 
elseif redis:get(bot_id.."Olifay:Lock:User:Name"..msg.chat_id_) == "kick" and not Vips(msg) then    
KickGroup(msg.chat_id_,msg.sender_user_id_)
Delete_Message(msg.chat_id_,{[0] = msg.id_}) 
elseif redis:get(bot_id.."Olifay:Lock:User:Name"..msg.chat_id_) == "ktm" and not Vips(msg) then    
redis:sadd(bot_id.."Olifay:Silence:User:Group"..msg.chat_id_,msg.sender_user_id_)
Delete_Message(msg.chat_id_,{[0] = msg.id_}) 
end
end
...
if msg.content_.caption_ then 
if msg.content_.caption_:match("#[%a%d_]+") or msg.content_.caption_:match("#(.+)") then 
if redis:get(bot_id.."Olifay:Lock:hashtak"..msg.chat_id_) == "del" and not Vips(msg) then    
Delete_Message(msg.chat_id_,{[0] = msg.id_}) 
elseif redis:get(bot_id.."Olifay:Lock:hashtak"..msg.chat_id_) == "ked" and not Vips(msg) then    
RestrictChat(msg.chat_id_,msg.sender_user_id_)
Delete_Message(msg.chat_id_,{[0] = msg.id_}) 
elseif redis:get(bot_id.."Olifay:Lock:hashtak"..msg.chat_id_) == "kick" and not Vips(msg) then    
KickGroup(msg.chat_id_,msg.sender_user_id_)
Delete_Message(msg.chat_id_,{[0] = msg.id_}) 
elseif redis:get(bot_id.."Olifay:Lock:hashtak"..msg.chat_id_) == "ktm" and not Vips(msg) then    
redis:sadd(bot_id.."Olifay:Silence:User:Group"..msg.chat_id_,msg.sender_user_id_)
Delete_Message(msg.chat_id_,{[0] = msg.id_}) 
end
end
...
if msg.content_.caption_ then 
if msg.content_.caption_:match("/[%a%d_]+") or msg.content_.caption_:match("/(.+)") then  
if redis:get(bot_id.."Olifay:Lock:Cmd"..msg.chat_id_) == "del" and not Vips(msg) then    
Delete_Message(msg.chat_id_,{[0] = msg.id_}) 
elseif redis:get(bot_id.."Olifay:Lock:Cmd"..msg.chat_id_) == "ked" and not Vips(msg) then    
RestrictChat(msg.chat_id_,msg.sender_user_id_)
Delete_Message(msg.chat_id_,{[0] = msg.id_}) 
elseif redis:get(bot_id.."Olifay:Lock:Cmd"..msg.chat_id_) == "kick" and not Vips(msg) then    
KickGroup(msg.chat_id_,msg.sender_user_id_)
Delete_Message(msg.chat_id_,{[0] = msg.id_}) 
elseif redis:get(bot_id.."Olifay:Lock:Cmd"..msg.chat_id_) == "ktm" and not Vips(msg) then    
redis:sadd(bot_id.."Olifay:Silence:User:Group"..msg.chat_id_,msg.sender_user_id_)
Delete_Message(msg.chat_id_,{[0] = msg.id_}) 
end
end
...
if redis:get(bot_id.."Olifay:Lock:Link"..msg.chat_id_) == "del" and not Vips(msg) then
Delete_Message(msg.chat_id_,{[0] = msg.id_}) 
elseif redis:get(bot_id.."Olifay:Lock:Link"..msg.chat_id_) == "ked" and not Vips(msg) then
RestrictChat(msg.chat_id_,msg.sender_user_id_)
Delete_Message(msg.chat_id_,{[0] = msg.id_}) 
elseif redis:get(bot_id.."Olifay:Lock:Link"..msg.chat_id_) == "kick" and not Vips(msg) then
KickGroup(msg.chat_id_,msg.sender_user_id_)
Delete_Message(msg.chat_id_,{[0] = msg.id_}) 
elseif redis:get(bot_id.."Olifay:Lock:Link"..msg.chat_id_) == "ktm" and not Vips(msg) then
redis:sadd(bot_id.."Olifay:Silence:User:Group"..msg.chat_id_,msg.sender_user_id_)
Delete_Message(msg.chat_id_,{[0] = msg.id_}) 
end
end
...
if redis:get(bot_id.."Olifay:Lock:Photo"..msg.chat_id_) == "del" then
Delete_Message(msg.chat_id_,{[0] = msg.id_}) 
elseif redis:get(bot_id.."Olifay:Lock:Photo"..msg.chat_id_) == "ked" then
RestrictChat(msg.chat_id_,msg.sender_user_id_)
Delete_Message(msg.chat_id_,{[0] = msg.id_}) 
elseif redis:get(bot_id.."Olifay:Lock:Photo"..msg.chat_id_) == "kick" then
KickGroup(msg.chat_id_,msg.sender_user_id_)
Delete_Message(msg.chat_id_,{[0] = msg.id_}) 
elseif redis:get(bot_id.."Olifay:Lock:Photo"..msg.chat_id_) == "ktm" then
redis:sadd(bot_id.."Olifay:Silence:User:Group"..msg.chat_id_,msg.sender_user_id_)
Delete_Message(msg.chat_id_,{[0] = msg.id_}) 
end
end
...
if redis:get(bot_id.."Olifay:Lock:Video"..msg.chat_id_) == "del" then
Delete_Message(msg.chat_id_,{[0] = msg.id_}) 
elseif redis:get(bot_id.."Olifay:Lock:Video"..msg.chat_id_) == "ked" then
RestrictChat(msg.chat_id_,msg.sender_user_id_)
Delete_Message(msg.chat_id_,{[0] = msg.id_}) 
elseif redis:get(bot_id.."Olifay:Lock:Video"..msg.chat_id_) == "kick" then
KickGroup(msg.chat_id_,msg.sender_user_id_)
Delete_Message(msg.chat_id_,{[0] = msg.id_}) 
elseif redis:get(bot_id.."Olifay:Lock:Video"..msg.chat_id_) == "ktm" then
redis:sadd(bot_id.."Olifay:Silence:User:Group"..msg.chat_id_,msg.sender_user_id_)
Delete_Message(msg.chat_id_,{[0] = msg.id_}) 
end
end
...
if redis:get(bot_id.."Olifay:Lock:Animation"..msg.chat_id_) == "del" then
Delete_Message(msg.chat_id_,{[0] = msg.id_}) 
elseif redis:get(bot_id.."Olifay:Lock:Animation"..msg.chat_id_) == "ked" then
RestrictChat(msg.chat_id_,msg.sender_user_id_)
Delete_Message(msg.chat_id_,{[0] = msg.id_}) 
elseif redis:get(bot_id.."Olifay:Lock:Animation"..msg.chat_id_) == "kick" then
KickGroup(msg.chat_id_,msg.sender_user_id_)
Delete_Message(msg.chat_id_,{[0] = msg.id_}) 
elseif redis:get(bot_id.."Olifay:Lock:Animation"..msg.chat_id_) == "ktm" then
redis:sadd(bot_id.."Olifay:Silence:User:Group"..msg.chat_id_,msg.sender_user_id_)
Delete_Message(msg.chat_id_,{[0] = msg.id_}) 
end
end
...
if redis:get(bot_id.."Olifay:Lock:geam"..msg.chat_id_) == "del" then
Delete_Message(msg.chat_id_,{[0] = msg.id_}) 
elseif redis:get(bot_id.."Olifay:Lock:geam"..msg.chat_id_) == "ked" then
RestrictChat(msg.chat_id_,msg.sender_user_id_)
Delete_Message(msg.chat_id_,{[0] = msg.id_}) 
elseif redis:get(bot_id.."Olifay:Lock:geam"..msg.chat_id_) == "kick" then
KickGroup(msg.chat_id_,msg.sender_user_id_)
Delete_Message(msg.chat_id_,{[0] = msg.id_}) 
elseif redis:get(bot_id.."Olifay:Lock:geam"..msg.chat_id_) == "ktm" then
redis:sadd(bot_id.."Olifay:Silence:User:Group"..msg.chat_id_,msg.sender_user_id_)
Delete_Message(msg.chat_id_,{[0] = msg.id_}) 
end
end
...
if redis:get(bot_id.."Olifay:Lock:Audio"..msg.chat_id_) == "del" then
Delete_Message(msg.chat_id_,{[0] = msg.id_}) 
elseif redis:get(bot_id.."Olifay:Lock:Audio"..msg.chat_id_) == "ked" then
RestrictChat(msg.chat_id_,msg.sender_user_id_)
Delete_Message(msg.chat_id_,{[0] = msg.id_}) 
elseif redis:get(bot_id.."Olifay:Lock:Audio"..msg.chat_id_) == "kick" then
KickGroup(msg.chat_id_,msg.sender_user_id_)
Delete_Message(msg.chat_id_,{[0] = msg.id_}) 
elseif redis:get(bot_id.."Olifay:Lock:Audio"..msg.chat_id_) == "ktm" then
redis:sadd(bot_id.."Olifay:Silence:User:Group"..msg.chat_id_,msg.sender_user_id_)
Delete_Message(msg.chat_id_,{[0] = msg.id_}) 
end
end
...
if redis:get(bot_id.."Olifay:Lock:vico"..msg.chat_id_) == "del" then
Delete_Message(msg.chat_id_,{[0] = msg.id_}) 
elseif redis:get(bot_id.."Olifay:Lock:vico"..msg.chat_id_) == "ked" then
RestrictChat(msg.chat_id_,msg.sender_user_id_)
Delete_Message(msg.chat_id_,{[0] = msg.id_}) 
elseif redis:get(bot_id.."Olifay:Lock:vico"..msg.chat_id_) == "kick" then
KickGroup(msg.chat_id_,msg.sender_user_id_)
Delete_Message(msg.chat_id_,{[0] = msg.id_}) 
elseif redis:get(bot_id.."Olifay:Lock:vico"..msg.chat_id_) == "ktm" then
redis:sadd(bot_id.."Olifay:Silence:User:Group"..msg.chat_id_,msg.sender_user_id_)
Delete_Message(msg.chat_id_,{[0] = msg.id_}) 
end
end
...
if redis:get(bot_id.."Olifay:Lock:Keyboard"..msg.chat_id_) == "del" then
Delete_Message(msg.chat_id_,{[0] = msg.id_}) 
elseif redis:get(bot_id.."Olifay:Lock:Keyboard"..msg.chat_id_) == "ked" then
RestrictChat(msg.chat_id_,msg.sender_user_id_)
Delete_Message(msg.chat_id_,{[0] = msg.id_}) 
elseif redis:get(bot_id.."Olifay:Lock:Keyboard"..msg.chat_id_) == "kick" then
KickGroup(msg.chat_id_,msg.sender_user_id_)
Delete_Message(msg.chat_id_,{[0] = msg.id_}) 
elseif redis:get(bot_id.."Olifay:Lock:Keyboard"..msg.chat_id_) == "ktm" then
redis:sadd(bot_id.."Olifay:Silence:User:Group"..msg.chat_id_,msg.sender_user_id_)
Delete_Message(msg.chat_id_,{[0] = msg.id_}) 
end
end
...
if redis:get(bot_id.."Olifay:Lock:Sticker"..msg.chat_id_) == "del" then
Delete_Message(msg.chat_id_,{[0] = msg.id_}) 
elseif redis:get(bot_id.."Olifay:Lock:Sticker"..msg.chat_id_) == "ked" then
RestrictChat(msg.chat_id_,msg.sender_user_id_)
Delete_Message(msg.chat_id_,{[0] = msg.id_}) 
elseif redis:get(bot_id.."Olifay:Lock:Sticker"..msg.chat_id_) == "kick" then
KickGroup(msg.chat_id_,msg.sender_user_id_)
Delete_Message(msg.chat_id_,{[0] = msg.id_}) 
elseif redis:get(bot_id.."Olifay:Lock:Sticker"..msg.chat_id_) == "ktm" then
redis:sadd(bot_id.."Olifay:Silence:User:Group"..msg.chat_id_,msg.sender_user_id_)
Delete_Message(msg.chat_id_,{[0] = msg.id_}) 
end
end
...
if redis:get(bot_id.."Olifay:Lock:forward"..msg.chat_id_) == "del" then
Delete_Message(msg.chat_id_,{[0] = msg.id_}) 
return false
elseif redis:get(bot_id.."Olifay:Lock:forward"..msg.chat_id_) == "ked" then
RestrictChat(msg.chat_id_,msg.sender_user_id_)
Delete_Message(msg.chat_id_,{[0] = msg.id_}) 
return false
elseif redis:get(bot_id.."Olifay:Lock:forward"..msg.chat_id_) == "kick" then
KickGroup(msg.chat_id_,msg.sender_user_id_)
Delete_Message(msg.chat_id_,{[0] = msg.id_}) 
return false
elseif redis:get(bot_id.."Olifay:Lock:forward"..msg.chat_id_) == "ktm" then
redis:sadd(bot_id.."Olifay:Silence:User:Group"..msg.chat_id_,msg.sender_user_id_)
Delete_Message(msg.chat_id_,{[0] = msg.id_}) 
return false
end
...
if redis:get(bot_id.."Olifay:Lock:Document"..msg.chat_id_) == "del" then
Delete_Message(msg.chat_id_,{[0] = msg.id_}) 
elseif redis:get(bot_id.."Olifay:Lock:Document"..msg.chat_id_) == "ked" then
RestrictChat(msg.chat_id_,msg.sender_user_id_)
Delete_Message(msg.chat_id_,{[0] = msg.id_}) 
elseif redis:get(bot_id.."Olifay:Lock:Document"..msg.chat_id_) == "kick" then
KickGroup(msg.chat_id_,msg.sender_user_id_)
Delete_Message(msg.chat_id_,{[0] = msg.id_}) 
elseif redis:get(bot_id.."Olifay:Lock:Document"..msg.chat_id_) == "ktm" then
redis:sadd(bot_id.."Olifay:Silence:User:Group"..msg.chat_id_,msg.sender_user_id_)
Delete_Message(msg.chat_id_,{[0] = msg.id_}) 
end
end
...
if redis:get(bot_id.."Olifay:Lock:Unsupported"..msg.chat_id_) == "del" then
Delete_Message(msg.chat_id_,{[0] = msg.id_}) 
elseif redis:get(bot_id.."Olifay:Lock:Unsupported"..msg.chat_id_) == "ked" then
RestrictChat(msg.chat_id_,msg.sender_user_id_)
Delete_Message(msg.chat_id_,{[0] = msg.id_}) 
elseif redis:get(bot_id.."Olifay:Lock:Unsupported"..msg.chat_id_) == "kick" then
KickGroup(msg.chat_id_,msg.sender_user_id_)
Delete_Message(msg.chat_id_,{[0] = msg.id_}) 
elseif redis:get(bot_id.."Olifay:Lock:Unsupported"..msg.chat_id_) == "ktm" then
redis:sadd(bot_id.."Olifay:Silence:User:Group"..msg.chat_id_,msg.sender_user_id_)
Delete_Message(msg.chat_id_,{[0] = msg.id_}) 
end
end
...
if not Vips(msg) then
if redis:get(bot_id.."Olifay:Lock:Markdaun"..msg.chat_id_) == "del" then
Delete_Message(msg.chat_id_,{[0] = msg.id_}) 
elseif redis:get(bot_id.."Olifay:Lock:Markdaun"..msg.chat_id_) == "ked" then
RestrictChat(msg.chat_id_,msg.sender_user_id_)
Delete_Message(msg.chat_id_,{[0] = msg.id_}) 
elseif redis:get(bot_id.."Olifay:Lock:Markdaun"..msg.chat_id_) == "kick" then
KickGroup(msg.chat_id_,msg.sender_user_id_)
Delete_Message(msg.chat_id_,{[0] = msg.id_}) 
elseif redis:get(bot_id.."Olifay:Lock:Markdaun"..msg.chat_id_) == "ktm" then
redis:sadd(bot_id.."Olifay:Silence:User:Group"..msg.chat_id_,msg.sender_user_id_)
Delete_Message(msg.chat_id_,{[0] = msg.id_}) 
end
end  
...
if redis:get(bot_id.."Olifay:Lock:Inlen"..msg.chat_id_) == "del" then
Delete_Message(msg.chat_id_,{[0] = msg.id_}) 
elseif redis:get(bot_id.."Olifay:Lock:Inlen"..msg.chat_id_) == "ked" then
RestrictChat(msg.chat_id_,msg.sender_user_id_)
Delete_Message(msg.chat_id_,{[0] = msg.id_}) 
elseif redis:get(bot_id.."Olifay:Lock:Inlen"..msg.chat_id_) == "kick" then
KickGroup(msg.chat_id_,msg.sender_user_id_)
Delete_Message(msg.chat_id_,{[0] = msg.id_}) 
elseif redis:get(bot_id.."Olifay:Lock:Inlen"..msg.chat_id_) == "ktm" then
redis:sadd(bot_id.."Olifay:Silence:User:Group"..msg.chat_id_,msg.sender_user_id_)
Delete_Message(msg.chat_id_,{[0] = msg.id_}) 
end
end 
...
if redis:get(bot_id.."Olifay:Lock:Contact"..msg.chat_id_) == "del" then
Delete_Message(msg.chat_id_,{[0] = msg.id_}) 
elseif redis:get(bot_id.."Olifay:Lock:Contact"..msg.chat_id_) == "ked" then
RestrictChat(msg.chat_id_,msg.sender_user_id_)
Delete_Message(msg.chat_id_,{[0] = msg.id_}) 
elseif redis:get(bot_id.."Olifay:Lock:Contact"..msg.chat_id_) == "kick" then
KickGroup(msg.chat_id_,msg.sender_user_id_)
Delete_Message(msg.chat_id_,{[0] = msg.id_}) 
elseif redis:get(bot_id.."Olifay:Lock:Contact"..msg.chat_id_) == "ktm" then
redis:sadd(bot_id.."Olifay:Silence:User:Group"..msg.chat_id_,msg.sender_user_id_)
Delete_Message(msg.chat_id_,{[0] = msg.id_}) 
end
end
...
sens = 400  
if redis:get(bot_id.."Olifay:Lock:Spam"..msg.chat_id_) == "del" and string.len(msg.content_.text_) > (sens) or ctrl_ > (sens) or real_ > (sens) then 
Delete_Message(msg.chat_id_,{[0] = msg.id_}) 
elseif redis:get(bot_id.."Olifay:Lock:Spam"..msg.chat_id_) == "ked" and string.len(msg.content_.text_) > (sens) or ctrl_ > (sens) or real_ > (sens) then 
RestrictChat(msg.chat_id_,msg.sender_user_id_)
Delete_Message(msg.chat_id_,{[0] = msg.id_}) 
elseif redis:get(bot_id.."Olifay:Lock:Spam"..msg.chat_id_) == "kick" and string.len(msg.content_.text_) > (sens) or ctrl_ > (sens) or real_ > (sens) then 
KickGroup(msg.chat_id_,msg.sender_user_id_)
Delete_Message(msg.chat_id_,{[0] = msg.id_}) 
elseif redis:get(bot_id.."Olifay:Lock:Spam"..msg.chat_id_) == "ktm" and string.len(msg.content_.text_) > (sens) or ctrl_ > (sens) or real_ > (sens) then 
redis:sadd(bot_id.."Olifay:Silence:User:Group"..msg.chat_id_,msg.sender_user_id_)
Delete_Message(msg.chat_id_,{[0] = msg.id_}) 
end
end
--------------------------------------------------------------------------------------------------------------
local status_welcome = redis:get(bot_id.."Olifay:Chek:Welcome"..msg.chat_id_)
if status_welcome and not redis:get(bot_id.."Olifay:Lock:tagservr"..msg.chat_id_) then
if msg.content_.ID == "MessageChatJoinByLink" then
tdcli_function({ID = "GetUser",user_id_=msg.sender_user_id_},function(extra,result) 
local GetWelcomeGroup = redis:get(bot_id.."Olifay:Get:Welcome:Group"..msg.chat_id_)  
...
redis:set(bot_id.."Olifay:Get:Id:Msg:Pin"..msg.chat_id_,msg.content_.message_id_)
else
local Msg_Pin = redis:get(bot_id.."Olifay:Get:Id:Msg:Pin"..msg.chat_id_)
if Msg_Pin and redis:get(bot_id.."Olifay:lockpin"..msg.chat_id_) then
Pin_Message(msg.chat_id_,Msg_Pin)
end
end
end
--------------------------------------------------------------------------------------------------------------
if not Vips(msg) and msg.content_.ID ~= "MessageChatAddMembers" and redis:hget(bot_id.."Olifay:Spam:Group:User"..msg.chat_id_,"Spam:User") then 
if msg.sender_user_id_ ~= bot_id then
floods = redis:hget(bot_id.."Olifay:Spam:Group:User"..msg.chat_id_,"Spam:User") or "nil"
Num_Msg_Max = redis:hget(bot_id.."Olifay:Spam:Group:User"..msg.chat_id_,"Num:Spam") or 5
Time_Spam = redis:hget(bot_id.."Olifay:Spam:Group:User"..msg.chat_id_,"Num:Spam:Time") or 5
local post_count = tonumber(redis:get(bot_id.."Olifay:Spam:Cont"..msg.sender_user_id_..":"..msg.chat_id_) or 0)
if post_count > tonumber(redis:hget(bot_id.."Olifay:Spam:Group:User"..msg.chat_id_,"Num:Spam") or 5) then 
local ch = msg.chat_id_
local type = redis:hget(bot_id.."Olifay:Spam:Group:User"..msg.chat_id_,"Spam:User") 
NotSpam(msg,type)  
end
redis:setex(bot_id.."Olifay:Spam:Cont"..msg.sender_user_id_..":"..msg.chat_id_, tonumber(redis:hget(bot_id.."Olifay:Spam:Group:User"..msg.chat_id_,"Num:Spam:Time") or 3), post_count+1) 
local edit_id = data.text_ or "nil"  
Num_Msg_Max = 5
if redis:hget(bot_id.."Olifay:Spam:Group:User"..msg.chat_id_,"Num:Spam") then
Num_Msg_Max = redis:hget(bot_id.."Olifay:Spam:Group:User"..msg.chat_id_,"Num:Spam") 
end
if redis:hget(bot_id.."Olifay:Spam:Group:User"..msg.chat_id_,"Num:Spam:Time") then
Time_Spam = redis:hget(bot_id.."Olifay:Spam:Group:User"..msg.chat_id_,"Num:Spam:Time") 
end 
end
end 
--------------------------------------------------------------------------------------------------------------
if msg.content_.photo_ then  
if redis:get(bot_id.."Olifay:Set:Chat:Photo"..msg.chat_id_..":"..msg.sender_user_id_) then 
...
redis:del(bot_id.."Olifay:Set:Chat:Photo"..msg.chat_id_..":"..msg.sender_user_id_) 
return false 
end
...
------------------------------------------------------------------------------------------------------------
if redis:get(bot_id.."Olifay:Broadcasting:Groups:Pin" .. msg.chat_id_ .. ":" .. msg.sender_user_id_) then 
print(text)
if text == "الغاء" or text == "الغاء ✖" then   
send(msg.chat_id_,msg.id_, "\nܛ تم الغاء الاذاعه للمجموعات") 
redis:del(bot_id.."Olifay:Broadcasting:Groups:Pin" .. msg.chat_id_ .. ":" .. msg.sender_user_id_) 
return false
end 
local list = redis:smembers(bot_id.."Olifay:Chek:Groups") 
send(msg.chat_id_, msg.id_,"ܛ تمت الاذاعه الى *- "..#list.." * مجموعه في البوت ")     
if msg.content_.text_ then
for k,v in pairs(list) do 
send(v, 0,"["..msg.content_.text_.."]")  
redis:set(bot_id..'Olifay:Msg:Pin:Chat'..v,msg.content_.text_) 
end
...
redis:set(bot_id..'Olifay:Msg:Pin:Chat'..v,photo) 
end 
elseif msg.content_.animation_ then
for k,v in pairs(list) do 
sendDocument(v, 0, msg.content_.animation_.animation_.persistent_id_,(msg.content_.caption_ or "")) 
redis:set(bot_id..'Olifay:Msg:Pin:Chat'..v,msg.content_.animation_.animation_.persistent_id_)
end 
elseif msg.content_.sticker_ then
for k,v in pairs(list) do 
sendSticker(v, 0, msg.content_.sticker_.sticker_.persistent_id_)   
redis:set(bot_id..'Olifay:Msg:Pin:Chat'..v,msg.content_.sticker_.sticker_.persistent_id_) 
end 
end
redis:del(bot_id.."Olifay:Broadcasting:Groups:Pin" .. msg.chat_id_ .. ":" .. msg.sender_user_id_) 
return false
...
------------------------------------------------------------------------------------------------------------
if redis:get(bot_id.."Olifay:Broadcasting:Users" .. msg.chat_id_ .. ":" .. msg.sender_user_id_) then 
if text == "الغاء" or text == "الغاء ✖" then   
send(msg.chat_id_,msg.id_, "\nܛ تم الغاء الاذاعه خاص") 
redis:del(bot_id.."Olifay:Broadcasting:Users" .. msg.chat_id_ .. ":" .. msg.sender_user_id_) 
return false
end 
local list = redis:smembers(bot_id..'Olifay:UsersBot')  
...
------------------------------------------------------------------------------------------------------------
if redis:get(bot_id.."Olifay:Broadcasting:Groups" .. msg.chat_id_ .. ":" .. msg.sender_user_id_) then 
if text == "الغاء" or text == "الغاء ✖" then   
send(msg.chat_id_,msg.id_, "\nܛ تم الغاء الاذاعه للمجموعات") 
redis:del(bot_id.."Olifay:Broadcasting:Groups" .. msg.chat_id_ .. ":" .. msg.sender_user_id_) 
return false
end 
local list = redis:smembers(bot_id.."Olifay:Chek:Groups") 
send(msg.chat_id_, msg.id_,"ܛ تمت الاذاعه الى *- "..#list.." * مجموعه في البوت ")     
...
------------------------------------------------------------------------------------------------------------
if redis:get(bot_id.."Olifay:Broadcasting:Groups:Fwd" .. msg.chat_id_ .. ":" .. msg.sender_user_id_) then 
if text == "الغاء" or text == "الغاء ✖" then   
send(msg.chat_id_,msg.id_, "\nܛ تم الغاء الاذاعه بالتوجيه للمجموعات") 
redis:del(bot_id.."Olifay:Broadcasting:Groups:Fwd" .. msg.chat_id_ .. ":" .. msg.sender_user_id_) 
return false  
end 
...
------------------------------------------------------------------------------------------------------------
if redis:get(bot_id.."Olifay:Broadcasting:Users:Fwd" .. msg.chat_id_ .. ":" .. msg.sender_user_id_) then 
if text == "الغاء" or text == "الغاء ✖" then   
send(msg.chat_id_,msg.id_, "\nܛ تم الغاء الاذاعه بالترجيه خاص") 
redis:del(bot_id.."Olifay:Broadcasting:Users:Fwd" .. msg.chat_id_ .. ":" .. msg.sender_user_id_) 
return false  
end 
if msg.forward_info_ then 
local list = redis:smembers(bot_id.."Olifay:UsersBot")   
...
------------------------------------------------------------------------------------------------------------
if redis:get(bot_id.."Olifay:Change:Description" .. msg.chat_id_ .. "" .. msg.sender_user_id_) then  
if text == "الغاء" then 
send(msg.chat_id_,msg.id_, "\nܛ تم الغاء امر تغير وصف المجموعه") 
redis:del(bot_id.."Olifay:Change:Description" .. msg.chat_id_ .. "" .. msg.sender_user_id_)
return false  
end 
redis:del(bot_id.."Olifay:Change:Description" .. msg.chat_id_ .. "" .. msg.sender_user_id_)   
...
--------------------------------------------------------------------------------------------------------------
if redis:get(bot_id.."Olifay:Welcome:Group" .. msg.chat_id_ .. "" .. msg.sender_user_id_) then 
if text == "الغاء" then 
send(msg.chat_id_,msg.id_, "\nܛ تم الغاء امر حفظ الترحيب") 
redis:del(bot_id.."Olifay:Welcome:Group" .. msg.chat_id_ .. "" .. msg.sender_user_id_)  
return false  
end 
redis:del(bot_id.."Olifay:Welcome:Group" .. msg.chat_id_ .. "" .. msg.sender_user_id_)  
redis:set(bot_id.."Olifay:Get:Welcome:Group"..msg.chat_id_,text) 
send(msg.chat_id_, msg.id_,"ܛ تم حفظ ترحيب المجموعه")   
return false   
end
--------------------------------------------------------------------------------------------------------------
if redis:get(bot_id.."Olifay:link:set"..msg.chat_id_..""..msg.sender_user_id_) then
if text == "الغاء" then
send(msg.chat_id_,msg.id_, "\nܛ تم الغاء امر حفظ الرابط") 
redis:del(bot_id.."Olifay:link:set"..msg.chat_id_..""..msg.sender_user_id_) 
return false
end
if text and text:match("(https://telegram.me/joinchat/%S+)") or text and text:match("(https://t.me/joinchat/%S+)") then     
local Link = text:match("(https://telegram.me/joinchat/%S+)") or text:match("(https://t.me/joinchat/%S+)")   
redis:set(bot_id.."Olifay:link:set:Group"..msg.chat_id_,Link)
...
local Text_Filter = redis:get(bot_id.."Olifay:Filter:Reply2"..text..msg.chat_id_)   
if Text_Filter then    
...
------------------------------------------------------------------------------------------------------------
if redis:get(bot_id.."Olifay:Change:Name:Bot"..msg.sender_user_id_) then 
if text == "الغاء" or text == "الغاء ✖" then   
send(msg.chat_id_,msg.id_, "\nܛ تم الغاء امر تغير اسم البوت") 
redis:del(bot_id.."Olifay:Change:Name:Bot"..msg.sender_user_id_) 
return false  
end 
redis:del(bot_id.."Olifay:Change:Name:Bot"..msg.sender_user_id_) 
redis:set(bot_id.."Olifay:Redis:Name:Bot",text) 
send(msg.chat_id_, msg.id_, "ܛ  تم تغير اسم البوت الى - "..text)  
return false
end 
------------------------------------------------------------------------------------------------------------
if redis:get(bot_id.."Olifay:Redis:Validity:Group"..msg.chat_id_..""..msg.sender_user_id_) then 
if text and text:match("^الغاء$") then 
send(msg.chat_id_,msg.id_, "\nܛ تم الغاء امر اضافة صلاحيه") 
local CmdDel = redis:get(bot_id.."Olifay:Add:Validity:Group:Rt:New"..msg.chat_id_..msg.sender_user_id_)  
redis:del(bot_id.."Olifay:Add:Validity:Group:Rt"..CmdDel..msg.chat_id_)
redis:srem(bot_id.."Olifay:Validitys:Group"..msg.chat_id_,CmdDel)  
redis:del(bot_id.."Olifay:Redis:Validity:Group"..msg.chat_id_..""..msg.sender_user_id_) 
return false  
end 
if text == "مدير" then
...
local textn = redis:get(bot_id.."Olifay:Add:Validity:Group:Rt:New"..msg.chat_id_..msg.sender_user_id_)  
redis:set(bot_id.."Olifay:Add:Validity:Group:Rt"..textn..msg.chat_id_,text)
...
------------------------------------------------------------------------------------------------------------
if redis:get(bot_id.."Olifay:Redis:Id:Group"..msg.chat_id_..""..msg.sender_user_id_) then 
if text == 'الغاء' then 
send(msg.chat_id_,msg.id_, "\nܛ تم الغاء امر تعين الايدي") 
redis:del(bot_id.."Olifay:Redis:Id:Group"..msg.chat_id_..""..msg.sender_user_id_) 
return false  
end 
redis:del(bot_id.."Olifay:Redis:Id:Group"..msg.chat_id_..""..msg.sender_user_id_) 
redis:set(bot_id.."Olifay:Set:Id:Group"..msg.chat_id_,text:match("(.*)"))
send(msg.chat_id_, msg.id_,'ܛ تم تعين الايدي الجديد')    
end
------------------------------------------------------------------------------------------------------------
if text == ""..(redis:get(bot_id.."Olifay:Random:Sm"..msg.chat_id_) or "").."" and not redis:get(bot_id.."Olifay:Set:Sma"..msg.chat_id_) then
if not redis:get(bot_id.."Olifay:Set:Sma"..msg.chat_id_) then 
send(msg.chat_id_, msg.id_,"\nܛ لقد فزت في اللعبه \nܛ اللعب مره اخره وارسل - سمايل او سمايلات")
redis:incrby(bot_id.."Olifay:Num:Add:Games"..msg.chat_id_..msg.sender_user_id_, 1)  
end
redis:set(bot_id.."Olifay:Set:Sma"..msg.chat_id_,true)
return false
end 
------------------------------------------------------------------------------------------------------------
if text == ""..(redis:get(bot_id.."Olifay:Klam:Speed"..msg.chat_id_) or "").."" and not redis:get(bot_id.."Olifay:Speed:Tr"..msg.chat_id_) then
if not redis:get(bot_id.."Olifay:Speed:Tr"..msg.chat_id_) then 
send(msg.chat_id_, msg.id_,"\nܛ لقد فزت في اللعبه \nܛ اللعب مره اخره وارسل - الاسرع او ترتيب")
redis:incrby(bot_id.."Olifay:Num:Add:Games"..msg.chat_id_..msg.sender_user_id_, 1)  
end
redis:set(bot_id.."Olifay:Speed:Tr"..msg.chat_id_,true)
end 
------------------------------------------------------------------------------------------------------------
if text == ""..(redis:get(bot_id.."Olifay:Klam:Hzor"..msg.chat_id_) or "").."" and not redis:get(bot_id.."Olifay:Set:Hzora"..msg.chat_id_) then
if not redis:get(bot_id.."Olifay:Set:Hzora"..msg.chat_id_) then 
send(msg.chat_id_, msg.id_,"\nܛ لقد فزت في اللعبه \nܛ اللعب مره اخره وارسل - حزوره")
redis:incrby(bot_id.."Olifay:Num:Add:Games"..msg.chat_id_..msg.sender_user_id_, 1)  
end
redis:set(bot_id.."Olifay:Set:Hzora"..msg.chat_id_,true)
end 
------------------------------------------------------------------------------------------------------------
if text == ""..(redis:get(bot_id.."Olifay:Maany"..msg.chat_id_) or "").."" and not redis:get(bot_id.."Olifay:Set:Maany"..msg.chat_id_) then
if not redis:get(bot_id.."Olifay:Set:Maany"..msg.chat_id_) then 
send(msg.chat_id_, msg.id_,"\nܛ لقد فزت في اللعبه \nܛ اللعب مره اخره وارسل - معاني")
redis:incrby(bot_id.."Olifay:Num:Add:Games"..msg.chat_id_..msg.sender_user_id_, 1)  
end
redis:set(bot_id.."Olifay:Set:Maany"..msg.chat_id_,true)
end 
------------------------------------------------------------------------------------------------------------
if text == ""..(redis:get(bot_id.."Olifay:Set:Aks:Game"..msg.chat_id_) or "").."" and not redis:get(bot_id.."Olifay:Set:Aks"..msg.chat_id_) then
if not redis:get(bot_id.."Olifay:Set:Aks"..msg.chat_id_) then 
send(msg.chat_id_, msg.id_,"\nܛ لقد فزت في اللعبه \nܛ اللعب مره اخره وارسل - العكس")
redis:incrby(bot_id.."Olifay:Num:Add:Games"..msg.chat_id_..msg.sender_user_id_, 1)  
end
redis:set(bot_id.."Olifay:Set:Aks"..msg.chat_id_,true)
end 
------------------------------------------------------------------------------------------------------------
if redis:get(bot_id.."Olifay:GAME:TKMEN" .. msg.chat_id_ .. "" .. msg.sender_user_id_) then  
...
local GETNUM = redis:get(bot_id.."Olifay:GAMES:NUM"..msg.chat_id_)
if tonumber(NUM) == tonumber(GETNUM) then
redis:del(bot_id.."Olifay:SADD:NUM"..msg.chat_id_..msg.sender_user_id_)
redis:del(bot_id.."Olifay:GAME:TKMEN" .. msg.chat_id_ .. "" .. msg.sender_user_id_)   
redis:incrby(bot_id.."Olifay:Num:Add:Games"..msg.chat_id_..msg.sender_user_id_,5)  
send(msg.chat_id_, msg.id_,"ܛ مبروك فزت ويانه وخمنت الرقم الصحيح\nܛ تم اضافة { 5 } من النقاط \n")
elseif tonumber(NUM) ~= tonumber(GETNUM) then
redis:incrby(bot_id.."Olifay:SADD:NUM"..msg.chat_id_..msg.sender_user_id_,1)
if tonumber(redis:get(bot_id.."Olifay:SADD:NUM"..msg.chat_id_..msg.sender_user_id_)) >= 3 then
redis:del(bot_id.."Olifay:SADD:NUM"..msg.chat_id_..msg.sender_user_id_)
redis:del(bot_id.."Olifay:GAME:TKMEN" .. msg.chat_id_ .. "" .. msg.sender_user_id_)   
...
------------------------------------------------------------------------------------------------------------
if redis:get(bot_id.."Olifay:SET:GAME" .. msg.chat_id_ .. "" .. msg.sender_user_id_) then  
if text and text:match("^(%d+)$") then
local NUM = text:match("^(%d+)$")
...
local GETNUM = redis:get(bot_id.."Olifay:Games:Bat"..msg.chat_id_)
if tonumber(NUM) == tonumber(GETNUM) then
redis:del(bot_id.."Olifay:SET:GAME" .. msg.chat_id_ .. "" .. msg.sender_user_id_)   
send(msg.chat_id_, msg.id_,"ܛ مبروك فزت وطلعت المحيبس بل ايد رقم { "..NUM.." }\nܛ لقد حصلت على { 3 }من نقاط يمكنك استبدالهن برسائل ")
redis:incrby(bot_id.."Olifay:Num:Add:Games"..msg.chat_id_..msg.sender_user_id_,3)  
elseif tonumber(NUM) ~= tonumber(GETNUM) then
redis:del(bot_id.."Olifay:SET:GAME" .. msg.chat_id_ .. "" .. msg.sender_user_id_)   
...
------------------------------------------------------------------------------------------------------------
if text == ""..(redis:get(bot_id.."Olifay::Set:Moktlf"..msg.chat_id_) or "").."" then 
if not redis:get(bot_id.."Olifay:Set:Moktlf:Bot"..msg.chat_id_) then 
redis:del(bot_id.."Olifay::Set:Moktlf"..msg.chat_id_)
send(msg.chat_id_, msg.id_,"\nܛ لقد فزت في اللعبه \nܛ اللعب مره اخره وارسل - المختلف")
redis:incrby(bot_id.."Olifay:Num:Add:Games"..msg.chat_id_..msg.sender_user_id_, 1)  
end
redis:set(bot_id.."Olifay:Set:Moktlf:Bot"..msg.chat_id_,true)
end
------------------------------------------------------------------------------------------------------------
if text == ""..(redis:get(bot_id.."Olifay:Set:Amth"..msg.chat_id_) or "").."" then 
if not redis:get(bot_id.."Olifay:Set:Amth:Bot"..msg.chat_id_) then 
redis:del(bot_id.."Olifay:Set:Amth"..msg.chat_id_)
send(msg.chat_id_, msg.id_,"\nܛ لقد فزت في اللعبه \nܛ اللعب مره اخره وارسل - امثله")
redis:incrby(bot_id.."Olifay:Num:Add:Games"..msg.chat_id_..msg.sender_user_id_, 1)  
end
redis:set(bot_id.."Olifay:Set:Amth:Bot"..msg.chat_id_,true)
end
------------------------------------------------------------------------------------------------------------
if redis:get(bot_id.."Olifay:Add:msg:user" .. msg.chat_id_ .. "" .. msg.sender_user_id_) then 
if text and text:match("^الغاء$") then 
redis:del(bot_id.."Olifay:id:user"..msg.chat_id_)  
send(msg.chat_id_,msg.id_, "\nܛ تم الغاء امر اضافة رسائل") 
redis:del(bot_id.."Olifay:Add:msg:user" .. msg.chat_id_ .. "" .. msg.sender_user_id_)  
return false  
end 
redis:del(bot_id.."Olifay:Add:msg:user" .. msg.chat_id_ .. "" .. msg.sender_user_id_)  
local numadded = string.match(text, "(%d+)") 
local iduserr = redis:get(bot_id.."Olifay:id:user"..msg.chat_id_)  
redis:del(bot_id.."Olifay:Msg_User"..msg.chat_id_..":"..msg.sender_user_id_) 
redis:incrby(bot_id.."Olifay:Num:Message:User"..msg.chat_id_..":"..iduserr,numadded)  
send(msg.chat_id_,msg.id_,"\nܛ تم اضافة له - "..numadded.." رسائل")  
end
------------------------------------------------------------------------------------------------------------
if redis:get(bot_id.."Olifay:games:add" .. msg.chat_id_ .. "" .. msg.sender_user_id_) then 
if text and text:match("^الغاء$") then 
redis:del(bot_id.."Olifay:idgem:user"..msg.chat_id_)  
send(msg.chat_id_,msg.id_, "\nܛ تم الغاء امر اضافة جواهر") 
redis:del(bot_id.."Olifay:games:add" .. msg.chat_id_ .. "" .. msg.sender_user_id_)  
return false  
end 
redis:del(bot_id.."Olifay:games:add" .. msg.chat_id_ .. "" .. msg.sender_user_id_)  
local numadded = string.match(text, "(%d+)") 
local iduserr = redis:get(bot_id.."Olifay:idgem:user"..msg.chat_id_)  
redis:incrby(bot_id.."Olifay:Num:Add:Games"..msg.chat_id_..iduserr,numadded)  
send(msg.chat_id_,msg.id_,"\nܛ تم اضافة له - "..numadded.." مجوهرات")  
end
------------------------------------------------------------------------------------------------------------
if redis:get(bot_id.."Olifay:Redis:Rules:" .. msg.chat_id_ .. ":" .. msg.sender_user_id_) then 
if text == "الغاء" then 
send(msg.chat_id_, msg.id_, "ܛ تم الغاء حفظ القوانين") 
redis:del(bot_id.."Olifay:Redis:Rules:" .. msg.chat_id_ .. ":" .. msg.sender_user_id_)
return false  
end 
redis:set(bot_id.."Olifay::Rules:Group" .. msg.chat_id_,text) 
send(msg.chat_id_, msg.id_,"ܛ تم حفظ قوانين المجموعه") 
redis:del(bot_id.."Olifay:Redis:Rules:" .. msg.chat_id_ .. ":" .. msg.sender_user_id_)
end  
------------------------------------------------------------------------------------------------------------
if text then 
local DelFilter = redis:get(bot_id.."Olifay:Filter:Reply1"..msg.sender_user_id_..msg.chat_id_)  
if DelFilter and DelFilter == "DelFilter" then   
send(msg.chat_id_, msg.id_,"ܛ تم الغاء منعها ")  
redis:del(bot_id.."Olifay:Filter:Reply1"..msg.sender_user_id_..msg.chat_id_)  
redis:del(bot_id.."Olifay:Filter:Reply2"..text..msg.chat_id_)  
redis:srem(bot_id.."Olifay:List:Filter"..msg.chat_id_,text)  
return false 
end  
end
------------------------------------------------------------------------------------------------------------
if text then   
local SetFilter = redis:get(bot_id.."Olifay:Filter:Reply1"..msg.sender_user_id_..msg.chat_id_)  
if SetFilter and SetFilter == "SetFilter" then   
send(msg.chat_id_, msg.id_,"ܛ ارسل التحذير عند ارسال الكلمه")  
redis:set(bot_id.."Olifay:Filter:Reply1"..msg.sender_user_id_..msg.chat_id_,"WirngFilter")  
redis:set(bot_id.."Olifay:Filter:Reply:Status"..msg.sender_user_id_..msg.chat_id_, text)  
redis:sadd(bot_id.."Olifay:List:Filter"..msg.chat_id_,text)  
return false  
end  
end
------------------------------------------------------------------------------------------------------------
if text then  
local WirngFilter = redis:get(bot_id.."Olifay:Filter:Reply1"..msg.sender_user_id_..msg.chat_id_)  
if WirngFilter and WirngFilter == "WirngFilter" then  
send(msg.chat_id_, msg.id_,"ܛ تم منع الكلمه مع التحذير")  
redis:del(bot_id.."Olifay:Filter:Reply1"..msg.sender_user_id_..msg.chat_id_)  
local test = redis:get(bot_id.."Olifay:Filter:Reply:Status"..msg.sender_user_id_..msg.chat_id_)  
if text then   
redis:set(bot_id.."Olifay:Filter:Reply2"..test..msg.chat_id_, text)  
end  
redis:del(bot_id.."Olifay:Filter:Reply:Status"..msg.sender_user_id_..msg.chat_id_)  
return false 
end  
end
------------------------------------------------------------------------------------------------------------
if text and redis:get(bot_id..'Olifay:GetTexting:DevStorm'..msg.chat_id_..':'..msg.sender_user_id_) then
if text == 'الغاء' or text == 'الغاء ✖' then 
redis:del(bot_id..'Olifay:GetTexting:DevStorm'..msg.chat_id_..':'..msg.sender_user_id_)
send(msg.chat_id_,msg.id_,'ܛ تم الغاء حفظ كليشة المطور')
return false
end
redis:set(bot_id..'Olifay:Texting:DevStorm',text)
redis:del(bot_id..'Olifay:GetTexting:DevStorm'..msg.chat_id_..':'..msg.sender_user_id_)
...
local test = redis:get(bot_id.."Olifay:Text:Manager"..msg.sender_user_id_..":"..msg.chat_id_.."")
if redis:get(bot_id.."Olifay:Set:Manager:rd"..msg.sender_user_id_..":"..msg.chat_id_) == "true1" then
redis:del(bot_id.."Olifay:Set:Manager:rd"..msg.sender_user_id_..":"..msg.chat_id_)
if msg.content_.sticker_ then   
redis:set(bot_id.."Olifay:Add:Rd:Manager:Stekrs"..test..msg.chat_id_, msg.content_.sticker_.sticker_.persistent_id_)  
end   
if msg.content_.voice_ then  
redis:set(bot_id.."Olifay:Add:Rd:Manager:Vico"..test..msg.chat_id_, msg.content_.voice_.voice_.persistent_id_)  
end   
if msg.content_.animation_ then   
redis:set(bot_id.."Olifay:Add:Rd:Manager:Gif"..test..msg.chat_id_, msg.content_.animation_.animation_.persistent_id_)  
end  
if text then   
text = text:gsub('"',"") 
text = text:gsub('"',"") 
text = text:gsub("`","") 
text = text:gsub("*","") 
redis:set(bot_id.."Olifay:Add:Rd:Manager:Text"..test..msg.chat_id_, text)  
end  
if msg.content_.audio_ then
redis:set(bot_id.."Olifay:Add:Rd:Manager:Audio"..test..msg.chat_id_, msg.content_.audio_.audio_.persistent_id_)  
end
if msg.content_.document_ then
redis:set(bot_id.."Olifay:Add:Rd:Manager:File"..test..msg.chat_id_, msg.content_.document_.document_.persistent_id_)  
end
if msg.content_.video_ then
redis:set(bot_id.."Olifay:Add:Rd:Manager:Video"..test..msg.chat_id_, msg.content_.video_.video_.persistent_id_)  
end
if msg.content_.photo_ then
if msg.content_.photo_.sizes_[0] then
...
redis:set(bot_id.."Olifay:Add:Rd:Manager:Photo"..test..msg.chat_id_, photo_in_group)  
end
...
if redis:get(bot_id.."Olifay:Set:Manager:rd"..msg.sender_user_id_..":"..msg.chat_id_) == "true" then
send(msg.chat_id_, msg.id_, '\nܛ ارسل لي الرد لاضافته\nܛ تستطيع اضافة ← { ملف ، فديو ، نص ، ملصق ، بصمه ، متحركه }\nܛ تستطيع ايضا اضافة :\nܛ `#username` » معرف المستخدم \nܛ `#msgs` » عدد الرسائل\nܛ `#name` » اسم المستخدم\nܛ `#id` » ايدي المستخدم\nܛ `#stast` » موقع المستخدم \nܛ `#edit` » عدد السحكات ')
redis:set(bot_id.."Olifay:Set:Manager:rd"..msg.sender_user_id_..":"..msg.chat_id_,"true1")
redis:set(bot_id.."Olifay:Text:Manager"..msg.sender_user_id_..":"..msg.chat_id_, text)
redis:del(bot_id.."Olifay:Add:Rd:Manager:Gif"..text..msg.chat_id_)   
redis:del(bot_id.."Olifay:Add:Rd:Manager:Vico"..text..msg.chat_id_)   
redis:del(bot_id.."Olifay:Add:Rd:Manager:Stekrs"..text..msg.chat_id_)     
redis:del(bot_id.."Olifay:Add:Rd:Manager:Text"..text..msg.chat_id_)   
redis:del(bot_id.."Olifay:Add:Rd:Manager:Photo"..text..msg.chat_id_)
redis:del(bot_id.."Olifay:Add:Rd:Manager:Video"..text..msg.chat_id_)
redis:del(bot_id.."Olifay:Add:Rd:Manager:File"..text..msg.chat_id_)
redis:del(bot_id.."Olifay:Add:Rd:Manager:Audio"..text..msg.chat_id_)
redis:sadd(bot_id.."Olifay:List:Manager"..msg.chat_id_.."", text)
return false end
end
------------------------------------------------------------------------------------------------------------
if text and text:match("^(.*)$") then
if redis:get(bot_id.."Olifay:Set:Manager:rd"..msg.sender_user_id_..":"..msg.chat_id_.."") == "true2" then
...
redis:del(bot_id.."Olifay:Add:Rd:Manager:Audio"..text..msg.chat_id_)
redis:del(bot_id.."Olifay:Set:Manager:rd"..msg.sender_user_id_..":"..msg.chat_id_)
redis:srem(bot_id.."Olifay:List:Manager"..msg.chat_id_.."", text)
return false
end
end
------------------------------------------------------------------------------------------------------------
if text and not redis:get(bot_id.."Olifay:Reply:Manager"..msg.chat_id_) then
if not redis:sismember(bot_id..'Olifay:Spam_For_Bot'..msg.sender_user_id_,text) then
local anemi = redis:get(bot_id.."Olifay:Add:Rd:Manager:Gif"..text..msg.chat_id_)   
local veico = redis:get(bot_id.."Olifay:Add:Rd:Manager:Vico"..text..msg.chat_id_)   
local stekr = redis:get(bot_id.."Olifay:Add:Rd:Manager:Stekrs"..text..msg.chat_id_)     
local Text = redis:get(bot_id.."Olifay:Add:Rd:Manager:Text"..text..msg.chat_id_)   
local photo = redis:get(bot_id.."Olifay:Add:Rd:Manager:Photo"..text..msg.chat_id_)
local video = redis:get(bot_id.."Olifay:Add:Rd:Manager:Video"..text..msg.chat_id_)
local document = redis:get(bot_id.."Olifay:Add:Rd:Manager:File"..text..msg.chat_id_)
local audio = redis:get(bot_id.."Olifay:Add:Rd:Manager:Audio"..text..msg.chat_id_)
if Text then 
tdcli_function({ID="GetUser",user_id_=msg.sender_user_id_},function(arg,data)
local NumMsg = redis:get(bot_id..'Olifay:Num:Message:User'..msg.chat_id_..':'..msg.sender_user_id_) or 0
local TotalMsg = Total_message(NumMsg)
local Status_Gps = Get_Rank(msg.sender_user_id_,msg.chat_id_)
local NumMessageEdit = redis:get(bot_id..'Olifay:Num:Message:Edit'..msg.chat_id_..msg.sender_user_id_) or 0
...
send(msg.chat_id_, msg.id_, Text)
redis:sadd(bot_id.."Olifay:Spam_For_Bot"..msg.sender_user_id_,text) 
end,nil)
end
if stekr then 
...
------------------------------------------------------------------------------------------------------------
if text or msg.content_.sticker_ or msg.content_.voice_ or msg.content_.animation_ or msg.content_.audio_ or msg.content_.document_ or msg.content_.photo_ or msg.content_.video_ then  
local test = redis:get(bot_id.."Olifay:Text:Sudo:Bot"..msg.sender_user_id_..":"..msg.chat_id_)
if redis:get(bot_id.."Olifay:Set:Rd"..msg.sender_user_id_..":"..msg.chat_id_) == "true1" then
redis:del(bot_id.."Olifay:Set:Rd"..msg.sender_user_id_..":"..msg.chat_id_)
if msg.content_.sticker_ then   
redis:set(bot_id.."Olifay:Add:Rd:Sudo:stekr"..test, msg.content_.sticker_.sticker_.persistent_id_)  
end   
if msg.content_.voice_ then  
redis:set(bot_id.."Olifay:Add:Rd:Sudo:vico"..test, msg.content_.voice_.voice_.persistent_id_)  
end   
if msg.content_.animation_ then   
redis:set(bot_id.."Olifay:Add:Rd:Sudo:Gif"..test, msg.content_.animation_.animation_.persistent_id_)  
end  
if text then   
text = text:gsub('"',"") 
text = text:gsub('"',"") 
text = text:gsub("`","") 
text = text:gsub("*","") 
redis:set(bot_id.."Olifay:Add:Rd:Sudo:Text"..test, text)  
end  
if msg.content_.audio_ then
redis:set(bot_id.."Olifay:Add:Rd:Sudo:Audio"..test, msg.content_.audio_.audio_.persistent_id_)  
end
if msg.content_.document_ then
redis:set(bot_id.."Olifay:Add:Rd:Sudo:File"..test, msg.content_.document_.document_.persistent_id_)  
end
if msg.content_.video_ then
redis:set(bot_id.."Olifay:Add:Rd:Sudo:Video"..test, msg.content_.video_.video_.persistent_id_)  
end
if msg.content_.photo_ then
if msg.content_.photo_.sizes_[0] then
photo_in_group = msg.content_.photo_.sizes_[0].photo_.persistent_id_
...
redis:set(bot_id.."Olifay:Add:Rd:Sudo:Photo"..test, photo_in_group)  
end
...
------------------------------------------------------------------------------------------------------------
if text and text:match("^(.*)$") then
if redis:get(bot_id.."Olifay:Set:Rd"..msg.sender_user_id_..":"..msg.chat_id_) == "true" then
send(msg.chat_id_, msg.id_, '\nܛ ارسل لي الكلمه الان \nܛ تستطيع اضافة ← { ملف ، فديو ، نص ، ملصق ، بصمه ، متحركه }\nܛ تستطيع ايضا اضافة :\nܛ `#username` » معرف المستخدم \nܛ `#msgs` » عدد الرسائل\nܛ `#name` » اسم المستخدم\nܛ `#id` » ايدي المستخدم\nܛ `#stast` » موقع المستخدم \nܛ `#edit` » عدد السحكات ')
redis:set(bot_id.."Olifay:Set:Rd"..msg.sender_user_id_..":"..msg.chat_id_, "true1")
redis:set(bot_id.."Olifay:Text:Sudo:Bot"..msg.sender_user_id_..":"..msg.chat_id_, text)
redis:sadd(bot_id.."Olifay:List:Rd:Sudo", text)
return false end
end
------------------------------------------------------------------------------------------------------------
if text and text:match("^(.*)$") then
if redis:get(bot_id.."Olifay:Set:On"..msg.sender_user_id_..":"..msg.chat_id_) == "true" then
send(msg.chat_id_, msg.id_,"ܛ تم حذف الرد من ردود المطور")
list = {"Add:Rd:Sudo:Audio","Add:Rd:Sudo:File","Add:Rd:Sudo:Video","Add:Rd:Sudo:Photo","Add:Rd:Sudo:Text","Add:Rd:Sudo:stekr","Add:Rd:Sudo:vico","Add:Rd:Sudo:Gif"}
for k,v in pairs(list) do
redis:del(bot_id..'Olifay:'..v..text)
end
redis:del(bot_id.."Olifay:Set:On"..msg.sender_user_id_..":"..msg.chat_id_)
redis:srem(bot_id.."Olifay:List:Rd:Sudo", text)
return false
end
end
if Dev_Olifay(msg) then
...
download_to_file('https://raw.githubusercontent.com/Remo4/Olifay/master/Olifay.lua','Olifay.lua') 
...
local Get_Files, res = https.request("https://raw.githubusercontent.com/Olifayy/Files_Olifay/master/getfile.json")
if res == 200 then
local Get_info, res = pcall(JSON.decode,Get_Files);
if Get_info then
local TextS = "\n📂┇اهلا بك في متجر ملفات Olifay \n📮┇الملفات الموجوده حاليا \nꔹ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ꔹ\n\n"
...
local Get_Json, Res = https.request("https://raw.githubusercontent.com/Olifayy/Files_Olifay/master/Files_Olifay/"..File_Name)
if Res == 200 then
...
------------------------------------------------------------------------------------------------------------
if text and not redis:get(bot_id.."Olifay:Reply:Sudo"..msg.chat_id_) then
if not redis:sismember(bot_id..'Olifay:Spam_For_Bot'..msg.sender_user_id_,text) then
local anemi = redis:get(bot_id.."Olifay:Add:Rd:Sudo:Gif"..text)   
local veico = redis:get(bot_id.."Olifay:Add:Rd:Sudo:vico"..text)   
local stekr = redis:get(bot_id.."Olifay:Add:Rd:Sudo:stekr"..text)     
local Text = redis:get(bot_id.."Olifay:Add:Rd:Sudo:Text"..text)   
local photo = redis:get(bot_id.."Olifay:Add:Rd:Sudo:Photo"..text)
local video = redis:get(bot_id.."Olifay:Add:Rd:Sudo:Video"..text)
local document = redis:get(bot_id.."Olifay:Add:Rd:Sudo:File"..text)
local audio = redis:get(bot_id.."Olifay:Add:Rd:Sudo:Audio"..text)
if Text then 
tdcli_function({ID="GetUser",user_id_=msg.sender_user_id_},function(arg,data)
local NumMsg = redis:get(bot_id..'Olifay:Num:Message:User'..msg.chat_id_..':'..msg.sender_user_id_) or 0
local TotalMsg = Total_message(NumMsg)
...
------------------------------------------------------------------------------------------------------------
if text and redis:get(bot_id..'Olifay:Set:Cmd:Start:Bots') then
if text == 'الغاء ✖' then   
send(msg.chat_id_, msg.id_,"ܛ تم الغاء حفظ كليشه امر /start") 
redis:del(bot_id..'Olifay:Set:Cmd:Start:Bots') 
return false
end
redis:set(bot_id.."Olifay:Set:Cmd:Start:Bot",text)  
...
if tonumber(data.member_count_) < tonumber(redis:get(bot_id..'Olifay:Num:Add:Bot') or 0) and not Dev_Olifay(msg) then
send(msg.chat_id_, msg.id_,'ܛ لا تستطيع تفعيل المجموعه بسبب قلة عدد اعضاء المجموعه يجب ان يكون اكثر من *:'..(redis:get(bot_id..'Olifay:Num:Add:Bot') or 0)..'* عضو')
return false
end
tdcli_function ({ID = "GetUser",user_id_ = msg.sender_user_id_},function(extra,result,success)
tdcli_function({ID ="GetChat",chat_id_=msg.chat_id_},function(arg,chat)  
if redis:sismember(bot_id..'Olifay:Chek:Groups',msg.chat_id_) then
send(msg.chat_id_, msg.id_,'ܛ تم تفعيل المجموعه مسبقا')
else
Send_Options(msg,result.id_,'reply_Add','ܛ تم تفعيل مجموعه '..chat.title_..'')
redis:sadd(bot_id..'Olifay:Chek:Groups',msg.chat_id_)
...
------------------------------------------------------------------------------------------------------------
if text == 'تفعيل' and not DeveloperBot(msg) and not redis:get(bot_id..'Olifay:Free:Bot') then
if TypeForChat ~= 'ForSuppur' then
send(msg.chat_id_, msg.id_,'ܛ المجموعه عاديه وليست خارقه لا تستطيع تفعيلي يرجى ان تضع سجل رسائل المجموعه ضاهر وليس مخفي ومن بعدها يمكنك رفعي ادمن ثم تفعيلي') 
return false
end
...
redis:sadd(bot_id..'Olifay:Chek:Groups',msg.chat_id_)  
redis:sadd(bot_id..'Olifay:Basic:Constructor'..msg.chat_id_, msg.sender_user_id_)
...
if not redis:get(bot_id..'Olifay:Ban:Cmd:Start'..msg.sender_user_id_) then
local GetCmdStart = redis:get(bot_id.."Olifay:Set:Cmd:Start:Bot")  
if not GetCmdStart then 
CmdStart = '\nܛأهلآ بك في بوت '..(redis:get(bot_id.."Olifay:Redis:Name:Bot") or "اولفاي")..''..
...
redis:setex(bot_id..'Olifay:Ban:Cmd:Start'..msg.sender_user_id_,60,true)
return false
end
...
redis:sadd(bot_id..'Olifay:User:Ban:Pv',data.id_)  
return false  
end
if text =='الغاء الحظر' then
Send_Optionspv(Id_Dev,msg.id_,UserForward,"reply_Pv","ܛ تم الغاء حظره من تواصل البوت")  
redis:srem(bot_id..'Olifay:User:Ban:Pv',data.id_)  
return false  
end 
...
redis:setex(bot_id.."Olifay:Broadcasting:Groups" .. msg.chat_id_ .. ":" .. msg.sender_user_id_, 600, true) 
send(msg.chat_id_, msg.id_,"ܛ ارسل لي المنشور الان\nܛ يمكنك ارسال -{ صوره - ملصق - متحركه - رساله }\nܛ لالغاء الاذاعه ارسل : الغاء") 
return false
end
if text=="اذاعه بالتثبيت 📌" and DeveloperBot(msg) then 
redis:setex(bot_id.."Olifay:Broadcasting:Groups:Pin" .. msg.chat_id_ .. ":" .. msg.sender_user_id_, 600, true) 
send(msg.chat_id_, msg.id_,"ܛ ارسل لي المنشور الان\nܛ يمكنك ارسال -{ صوره - ملصق - متحركه - رساله }\nܛ لالغاء الاذاعه ارسل : الغاء") 
return false
end
if text=="اذاعه بالتوجيه 👥" and DeveloperBot(msg) then 
redis:setex(bot_id.."Olifay:Broadcasting:Groups:Fwd" .. msg.chat_id_ .. ":" .. msg.sender_user_id_, 600, true) 
...
local list = redis:smembers(bot_id.."Olifay:Sudo:User")
...
if Dev_Olifay_User(result.id_) == true then
send(msg.chat_id_, msg.id_, "ܛ لا تستطيع حظر المطور الاساسي عام")
return false 
end
redis:sadd(bot_id.."Olifay:Removal:User:Groups", result.id_)
...
redis:srem(bot_id.."Olifay:Removal:User:Groups", result.id_)
else
send(msg.chat_id_, msg.id_,"ܛ المعرف غلط ")
end
end
...
redis:sadd(bot_id.."Olifay:Sudo:User", result.id_)
...
function FunctionStatus(arg, result)
if (result.id_) then
redis:srem(bot_id.."Olifay:Sudo:User", result.id_)
...
send(msg.chat_id_, msg.id_,'*ܛ عدد احصائيات البوت الكامله \n━━━━━━━━━━━━━\nܛ عدد المجموعات : '..(redis:scard(bot_id..'Olifay:Chek:Groups') or 0)..'\nܛ عدد المشتركين : '..(redis:scard(bot_id..'Olifay:UsersBot') or 0)..'*')
end
if text and text:match("^تعين عدد الاعضاء (%d+)$") then
redis:set(bot_id..'Olifay:Num:Add:Bot',text:match("تعين عدد الاعضاء (%d+)$") ) 
...
local pv = redis:smembers(bot_id..'Olifay:UsersBot')  
local sendok = 0
for i = 1, #pv do
...
redis:srem(bot_id..'Olifay:UsersBot',pv[i])  
sendok = sendok + 1
end
if #pv == i then 
if sendok == 0 then
...
local group = redis:smembers(bot_id..'Olifay:Chek:Groups')  
local w = 0
local q = 0
for i = 1, #group do
...
redis:srem(bot_id..'Olifay:Chek:Groups',group[i])  
w = w + 1
end
...
local Groups = redis:smembers(bot_id..'Olifay:Chek:Groups')  
local Get_Json = '{"IdBot": '..bot_id..'Olifay:,"Groups":{'  
for k,v in pairs(Groups) do   
local President = redis:smembers(bot_id.."Olifay:Basic:Constructor"..v)
local Constructor = redis:smembers(bot_id.."Olifay:Constructor"..v)
local Manager = redis:smembers(bot_id.."Olifay:Manager"..v)
local Admin = redis:smembers(bot_id.."Olifay:Mod:User"..v)
local Vips = redis:smembers(bot_id.."Olifay:Special:User"..v)
local LinkGroup = redis:get(bot_id.."Olifay:link:set:Group"..v) 
local WelcomeGroup = redis:get(bot_id.."Olifay:Get:Welcome:Group"..v) or ''
local Status_Dev = redis:get(bot_id.."Olifay:Sudo:User:Reply"..v) 
local Status_Prt = redis:get(bot_id.."Olifay:Basic:Constructor:Reply"..v) 
local Status_Cto = redis:get(bot_id.."Olifay:Constructor:Reply"..v) 
local Status_Own = redis:get(bot_id.."Olifay:Manager:Reply"..v) 
local Status_Md = redis:get(bot_id.."Olifay:Mod:User:Reply"..v) 
local Status_Vip = redis:get(bot_id.."Olifay:Special:User:Reply"..v) 
local Status_Mem = redis:get(bot_id.."Olifay:Mempar:Group:Reply"..v) 
if k == 1 then
Get_Json = Get_Json..'"'..v..'":{'
else
Get_Json = Get_Json..',"'..v..'":{'
...
redis:del(bot_id..'Olifay:Lock:Twasl') 
send(msg.chat_id_, msg.id_,'ܛ  تم تفعيل التواصل ') 
end
if text == 'تعطيل تواصل البوت 🔕' then  
redis:set(bot_id..'Olifay:Lock:Twasl',true) 
send(msg.chat_id_, msg.id_,'ܛ  تم تعطيل التواصل ') 
end
end 
end
if TypeForChat == ("ForSuppur") then
if text ==  ""..(redis:get(bot_id.."Olifay:Redis:Name:Bot") or "اولفاي")..' شنو رئيك بهاذا' and tonumber(msg.reply_to_message_id_) > 0 and not redis:get(bot_id.."Olifay:Fun:Bots"..msg.chat_id_) then     
...
if text == ""..(redis:get(bot_id.."Olifay:Redis:Name:Bot") or "اولفاي")..' شنو رئيك بهاي' and tonumber(msg.reply_to_message_id_) > 0 and not redis:get(bot_id.."Olifay:Fun:Bots"..msg.chat_id_) then    
tdcli_function ({ID = "GetMessage",chat_id_ = msg.chat_id_,message_id_ = tonumber(msg.reply_to_message_id_)},function(Arg, Data) 
...
if text == 'رفع النسخه الاحتياطيه' and tonumber(msg.reply_to_message_id_) > 0 and Dev_Olifay(msg) then   
tdcli_function({ID = "GetMessage",chat_id_=msg.chat_id_,message_id_=tonumber(msg.reply_to_message_id_)},function(Arg, Data)   
...
if text == 'جلب نسخه احتياطيه' and Dev_Olifay(msg) or text == 'جلب نسخه الكروبات' and Dev_Olifay(msg) then
local Groups = redis:smembers(bot_id..'Olifay:Chek:Groups')  
local Get_Json = '{"IdBot": '..bot_id..'Olifay:,"Groups":{'  
for k,v in pairs(Groups) do   
local President = redis:smembers(bot_id.."Olifay:Basic:Constructor"..v)
...
if text == ("مسح قائمه العام") and Dev_Olifay(msg) or text == ("مسح المحظورين عام") and Dev_Olifay(msg) then
redis:del(bot_id.."Olifay:Removal:User:Groups")
send(msg.chat_id_, msg.id_, "ܛتم مسح المحظورين عام من البوت")
end
if text == ("مسح المطورين") and Dev_Olifay(msg) then
redis:del(bot_id.."Olifay:Sudo:User")
send(msg.chat_id_, msg.id_, "ܛ تم مسح المطورين من البوت  ")
end
if text == ("مسح المنشئين الاساسين") and DeveloperBot(msg) or text == "مسح الاساسين" and DeveloperBot(msg)  then
redis:del(bot_id.."Olifay:Basic:Constructor"..msg.chat_id_)
...
local list = redis:smembers(bot_id.."Olifay:Command:List:Group"..msg.chat_id_)
for k,v in pairs(list) do
redis:del(bot_id.."Olifay:Get:Reides:Commands:Group"..msg.chat_id_..":"..v)
redis:del(bot_id.."Olifay:Command:List:Group"..msg.chat_id_)
end
send(msg.chat_id_, msg.id_,"ܛ تم مسح جميع الاوامر التي تم اضافتها")  
end
if text == "مسح الصلاحيات" and Constructor(msg) then
local list = redis:smembers(bot_id.."Olifay:Validitys:Group"..msg.chat_id_)
for k,v in pairs(list) do;redis:del(bot_id.."Olifay:Add:Validity:Group:Rt"..v..msg.chat_id_);redis:del(bot_id.."Olifay:Validitys:Group"..msg.chat_id_);end
send(msg.chat_id_, msg.id_,"ܛ تم مسح صلاحيات المجموعه")
end
if text == ("قائمه العام") and Dev_Olifay(msg) or text == ("المحظورين عام") and Dev_Olifay(msg) then
local list = redis:smembers(bot_id.."Olifay:Removal:User:Groups")
Gban = "\nܛقائمة المحظورين عام في البوت\n━━━━━━━━━━━━━\n"
for k,v in pairs(list) do
local username = redis:get(bot_id.."Olifay:Save:Username" .. v)
...
if text == ("المطورين") and Dev_Olifay(msg) then
local list = redis:smembers(bot_id.."Olifay:Sudo:User")
Sudos = "\nܛقائمة مطورين في البوت \n━━━━━━━━━━━━━\n"
for k,v in pairs(list) do
local username = redis:get(bot_id.."Olifay:Save:Username" .. v)
...
local list = redis:smembers(bot_id.."Olifay:Basic:Constructor"..msg.chat_id_)
...
local list = redis:smembers(bot_id.."Olifay:Constructor"..msg.chat_id_)
...
local list = redis:smembers(bot_id.."Olifay:Manager"..msg.chat_id_)
...
local list = redis:smembers(bot_id.."Olifay:Mod:User"..msg.chat_id_)
...
local list = redis:smembers(bot_id.."Olifay:Special:User"..msg.chat_id_)
...
local list = redis:smembers(bot_id.."Olifay:Silence:User:Group"..msg.chat_id_)
...
local list = redis:smembers(bot_id.."Olifay:Removal:User:Group"..msg.chat_id_)
...
for k,v in pairs(list) do
var = redis:get(bot_id.."Olifay:Add:Validity:Group:Rt"..v..msg.chat_id_)
if var then
...
for k,v in pairs(list) do
Commands = redis:get(bot_id.."Olifay:Get:Reides:Commands:Group"..msg.chat_id_..":"..v)
if Commands then 
...
for k, v in pairs(list) do
x = x + 1
if redis:get(bot_id.."Olifay:Save:Username"..v.user_id_) then
t = t..""..x.." : [@"..redis:get(bot_id.."Olifay:Save:Username"..v.user_id_).."]\n"
else
t = t..""..x.." : "..v.user_id_.."\n"
end
end
...
if text == ("حظر عام") and tonumber(msg.reply_to_message_id_) ~= 0 and Dev_Olifay(msg) then
function FunctionStatus(arg, result)
if tonumber(result.sender_user_id_) == tonumber(bot_id) then  
send(msg.chat_id_, msg.id_, "ܛ لا تسطيع حظر البوت عام")
return false 
end
if Dev_Olifay_User(result.sender_user_id_) == true then
send(msg.chat_id_, msg.id_, "ܛ لا تستطيع حظر المطور الاساسي عام")
return false 
end
Send_Options(msg,result.sender_user_id_,"reply","تم حظره عام من المجموعات")  
redis:sadd(bot_id.."Olifay:Removal:User:Groups", result.sender_user_id_)
KickGroup(result.chat_id_, result.sender_user_id_)
end
tdcli_function ({ID = "GetMessage",chat_id_ = msg.chat_id_,message_id_ = tonumber(msg.reply_to_message_id_)}, FunctionStatus, nil)
end
if text == ("الغاء العام") and tonumber(msg.reply_to_message_id_) ~= 0 and Dev_Olifay(msg) then
function FunctionStatus(arg, result)
redis:srem(bot_id.."Olifay:Removal:User:Groups", result.sender_user_id_)
Send_Options(msg,result.sender_user_id_,"reply","تم الغاء حظره عام من المجموعات")  
end
tdcli_function ({ID = "GetMessage",chat_id_ = msg.chat_id_,message_id_ = tonumber(msg.reply_to_message_id_)}, FunctionStatus, nil)
end
if text == ("اضف مطور") and tonumber(msg.reply_to_message_id_) ~= 0 and Dev_Olifay(msg) then
function FunctionStatus(arg, result)
redis:sadd(bot_id.."Olifay:Sudo:User", result.sender_user_id_)
Send_Options(msg,result.sender_user_id_,"reply","تم ترقيته مطور في البوت")  
end
tdcli_function ({ID = "GetMessage",chat_id_ = msg.chat_id_,message_id_ = tonumber(msg.reply_to_message_id_)}, FunctionStatus, nil)
end
if text == ("حذف مطور") and tonumber(msg.reply_to_message_id_) ~= 0 and Dev_Olifay(msg) then
function FunctionStatus(arg, result)
redis:srem(bot_id.."Olifay:Sudo:User", result.sender_user_id_)
...
function FunctionStatus(arg, result)
redis:sadd(bot_id.."Olifay:Basic:Constructor"..msg.chat_id_, result.sender_user_id_)
...
function FunctionStatus(arg, result)
redis:srem(bot_id.."Olifay:Basic:Constructor"..msg.chat_id_, result.sender_user_id_)
...
function FunctionStatus(arg, result)
redis:sadd(bot_id.."Olifay:Constructor"..msg.chat_id_, result.sender_user_id_)
...
function FunctionStatus(arg, result)
redis:srem(bot_id.."Olifay:Constructor"..msg.chat_id_, result.sender_user_id_)
Send_Options(msg,result.sender_user_id_,"reply","تم تنزيله من المنشئين")  
end
tdcli_function ({ID = "GetMessage",chat_id_ = msg.chat_id_,message_id_ = tonumber(msg.reply_to_message_id_)}, FunctionStatus, nil)
end
...
function FunctionStatus(arg, result)
redis:sadd(bot_id.."Olifay:Manager"..msg.chat_id_, result.sender_user_id_)
...
function FunctionStatus(arg, result)
redis:srem(bot_id.."Olifay:Manager"..msg.chat_id_, result.sender_user_id_)
Send_Options(msg,result.sender_user_id_,"reply","تم تنزيله من المدراء")  
end
tdcli_function ({ID = "GetMessage",chat_id_ = msg.chat_id_,message_id_ = tonumber(msg.reply_to_message_id_)}, FunctionStatus, nil)
end
if text == ("رفع ادمن") and tonumber(msg.reply_to_message_id_) ~= 0 and Owner(msg) then
if not Constructor(msg) and redis:get(bot_id.."Olifay:Cheking:Seted"..msg.chat_id_) then 
send(msg.chat_id_, msg.id_,'ܛلا تستطيع رفع احد وذالك لان تم تعطيل الرفع من قبل المنشئين')
return false
end
function FunctionStatus(arg, result)
redis:sadd(bot_id.."Olifay:Mod:User"..msg.chat_id_, result.sender_user_id_)
...
function FunctionStatus(arg, result)
redis:srem(bot_id.."Olifay:Mod:User"..msg.chat_id_, result.sender_user_id_)
...
function FunctionStatus(arg, result)
redis:sadd(bot_id.."Olifay:Special:User"..msg.chat_id_, result.sender_user_id_)
...
function FunctionStatus(arg, result)
redis:srem(bot_id.."Olifay:Special:User"..msg.chat_id_, result.sender_user_id_)
...
if not Constructor(msg) and redis:get(bot_id.."Olifay:Lock:Ban:Group"..msg.chat_id_) then 
...
redis:sadd(bot_id.."Olifay:Removal:User:Group"..msg.chat_id_, result.sender_user_id_)
KickGroup(result.chat_id_, result.sender_user_id_)
...
redis:srem(bot_id.."Olifay:Removal:User:Group"..msg.chat_id_, result.sender_user_id_)
...
redis:sadd(bot_id.."Olifay:Silence:User:Group"..msg.chat_id_, result.sender_user_id_)
...
function FunctionStatus(arg, result)
redis:srem(bot_id.."Olifay:Silence:User:Group"..msg.chat_id_, result.sender_user_id_)
...
if text and text:match("^حظر عام @(.*)$") and Dev_Olifay(msg) then
function FunctionStatus(arg, result)
if (result.id_) then
if result and result.type_ and result.type_.ID == ("ChannelChatInfo") then
send(msg.chat_id_,msg.id_,"ܛ عذرا هاذا معرف قناة")   
...
if text and text:match("^الغاء العام @(.*)$") and Dev_Olifay(msg) then
function FunctionStatus(arg, result)
if (result.id_) then
...
if text and text:match("^اضف مطور @(.*)$") and Dev_Olifay(msg) then
function FunctionStatus(arg, result)
if (result.id_) then
if (result and result.type_ and result.type_.ID == "ChannelChatInfo") then
send(msg.chat_id_,msg.id_,"ܛ عذرا هاذا معرف قناة")   
...
if text and text:match("^حذف مطور @(.*)$") and Dev_Olifay(msg) then
function FunctionStatus(arg, result)
if (result.id_) then
redis:srem(bot_id.."Olifay:Sudo:User", result.id_)
...
redis:sadd(bot_id.."Olifay:Basic:Constructor"..msg.chat_id_, result.id_)
...
function FunctionStatus(arg, result)
if (result.id_) then
redis:srem(bot_id.."Olifay:Basic:Constructor"..msg.chat_id_, result.id_)
...
redis:sadd(bot_id.."Olifay:Constructor"..msg.chat_id_, result.id_)
...
function FunctionStatus(arg, result)
if (result.id_) then
redis:srem(bot_id.."Olifay:Constructor"..msg.chat_id_, result.id_)
Send_Options(msg,result.id_,"reply"," تم تنزيله من المنشئين")  
else
send(msg.chat_id_, msg.id_,"ܛ المعرف غلط ")
end
...
redis:sadd(bot_id.."Olifay:Manager"..msg.chat_id_, result.id_)
...
function FunctionStatus(arg, result)
if (result.id_) then
redis:srem(bot_id.."Olifay:Manager"..msg.chat_id_, result.id_)
...
redis:sadd(bot_id.."Olifay:Mod:User"..msg.chat_id_, result.id_)
...
function FunctionStatus(arg, result)
if (result.id_) then
redis:srem(bot_id.."Olifay:Mod:User"..msg.chat_id_, result.id_)
...
redis:sadd(bot_id.."Olifay:Special:User"..msg.chat_id_, result.id_)
...
function FunctionStatus(arg, result)
if (result.id_) then
redis:srem(bot_id.."Olifay:Special:User"..msg.chat_id_, result.id_)
...
if redis:sismember(bot_id.."Olifay:Validitys:Group"..msg.chat_id_,text:match("رفع (.*)")) then
function Status_reply(extra, result, success)   
local statusrt = redis:get(bot_id.."Olifay:Add:Validity:Group:Rt"..text:match("رفع (.*)")..msg.chat_id_)
if statusrt == "مميز" and Admin(msg) then
redis:set(bot_id.."Olifay:Add:Validity:Users"..msg.chat_id_..result.sender_user_id_,text:match("رفع (.*)")) 
redis:sadd(bot_id.."Olifay:Special:User"..msg.chat_id_,result.sender_user_id_)  
...
if redis:sismember(bot_id.."Olifay:Validitys:Group"..msg.chat_id_,text:match("تنزيل (.*)")) then
function Status_reply(extra, result, success)   
local statusrt = redis:get(bot_id.."Olifay:Add:Validity:Group:Rt"..text:match("تنزيل (.*)")..msg.chat_id_)
if statusrt == "مميز" and Admin(msg) then
redis:srem(bot_id.."Olifay:Special:User"..msg.chat_id_,result.sender_user_id_)  
redis:del(bot_id.."Olifay:Add:Validity:Users"..msg.chat_id_..result.sender_user_id_)
elseif statusrt == "ادمن" and Owner(msg) then 
redis:srem(bot_id.."Olifay:Mod:User"..msg.chat_id_,result.sender_user_id_) 
redis:del(bot_id.."Olifay:Add:Validity:Users"..msg.chat_id_..result.sender_user_id_)
elseif statusrt == "مدير" and Constructor(msg) then
...
if redis:sismember(bot_id.."Olifay:Validitys:Group"..msg.chat_id_,text1[2]) then
function status_username(extra, result, success)   
if (result.id_) then
local statusrt = redis:get(bot_id.."Olifay:Add:Validity:Group:Rt"..text1[2]..msg.chat_id_)
if statusrt == "مميز" and Admin(msg) then
redis:sadd(bot_id.."Olifay:Special:User"..msg.chat_id_,result.id_)  
redis:set(bot_id.."Olifay:Add:Validity:Users"..msg.chat_id_..result.id_,text1[2])
end
...
if statusrt == "مميز" and Admin(msg) then
redis:srem(bot_id.."Olifay:Special:User"..msg.chat_id_,result.id_)  
redis:del(bot_id.."Olifay:Add:Validity:Users"..msg.chat_id_..result.id_)
end
if statusrt == "ادمن" and Owner(msg) then 
redis:srem(bot_id.."Olifay:Mod:User"..msg.chat_id_,result.id_)  
redis:del(bot_id.."Olifay:Add:Validity:Users"..msg.chat_id_..result.id_)
...
redis:sadd(bot_id.."Olifay:Removal:User:Group"..msg.chat_id_, result.id_)
...
redis:srem(bot_id.."Olifay:Removal:User:Group"..msg.chat_id_, result.id_)
...
redis:sadd(bot_id.."Olifay:Silence:User:Group"..msg.chat_id_, result.id_)
...
function FunctionStatus(arg, result)
if (result.id_) then
redis:srem(bot_id.."Olifay:Silence:User:Group"..msg.chat_id_, result.id_)
...
if text and text:match("^حظر عام (%d+)$") and Dev_Olifay(msg) then
if Dev_Olifay_User(text:match("^حظر عام (%d+)$")) == true then
send(msg.chat_id_, msg.id_, "ܛ لا تستطيع حظر المطور الاساسي عام")
return false 
end
...
redis:sadd(bot_id.."Olifay:Removal:User:Groups", text:match("^حظر عام (%d+)$"))
Send_Options(msg,text:match("^حظر عام (%d+)$"),"reply"," تم حظره عام من المجموعات")  
end
if text and text:match("^الغاء العام (%d+)$") and Dev_Olifay(msg) then
redis:srem(bot_id.."Olifay:Removal:User:Groups", text:match("^الغاء العام (%d+)$"))
Send_Options(msg,text:match("^الغاء العام (%d+)$"),"reply"," تم الغاء حظره عام من المجموعات")  
return false
end
if text and text:match("^اضف مطور (%d+)$") and Dev_Olifay(msg) then
redis:sadd(bot_id.."Olifay:Sudo:User", text:match("^اضف مطور (%d+)$"))
Send_Options(msg,text:match("^اضف مطور (%d+)$"),"reply"," تم ترقيته مطور في البوت")  
end
if text and text:match("^حذف مطور (%d+)$") and Dev_Olifay(msg) then
redis:srem(bot_id.."Olifay:Sudo:User", text:match("^حذف مطور (%d+)$"))
...
redis:sadd(bot_id.."Olifay:Basic:Constructor"..msg.chat_id_, text:match("^رفع منشئ اساسي (%d+)$") )
...
redis:srem(bot_id.."Olifay:Basic:Constructor"..msg.chat_id_, text:match("^تنزيل منشئ اساسي (%d+)$") )
...
redis:sadd(bot_id.."Olifay:Constructor"..msg.chat_id_, text:match("^رفع منشئ (%d+)$"))
...
redis:srem(bot_id.."Olifay:Constructor"..msg.chat_id_, text:match("^تنزيل منشئ (%d+)$"))
Send_Options(msg,text:match("^تنزيل منشئ (%d+)$"),"reply"," تم تنزيله من المنشئين")  
end
if text and text:match("^رفع مدير (%d+)$") and Constructor(msg) then
redis:sadd(bot_id.."Olifay:Manager"..msg.chat_id_, text:match("^رفع مدير (%d+)$") )
Send_Options(msg,text:match("^رفع مدير (%d+)$") ,"reply"," تم ترقيته مدير المجموعه")  
end
if text and text:match("^تنزيل مدير (%d+)$") and Constructor(msg) then
redis:srem(bot_id.."Olifay:Manager"..msg.chat_id_, text:match("^تنزيل مدير (%d+)$") )
...
redis:sadd(bot_id.."Olifay:Mod:User"..msg.chat_id_, text:match("^رفع ادمن (%d+)$"))
Send_Options(msg,text:match("^رفع ادمن (%d+)$"),"reply"," تم ترقيته ادمن للمجموعه")  
end
if text and text:match("^تنزيل ادمن (%d+)$") and Owner(msg) then
redis:srem(bot_id.."Olifay:Mod:User"..msg.chat_id_, text:match("^تنزيل ادمن (%d+)$"))
...
redis:sadd(bot_id.."Olifay:Special:User"..msg.chat_id_, text:match("^رفع مميز (%d+)$"))
...
redis:srem(bot_id.."Olifay:Special:User"..msg.chat_id_, text:match("^تنزيل مميز (%d+)$") )
...
redis:sadd(bot_id.."Olifay:Removal:User:Group"..msg.chat_id_, text:match("^حظر (%d+)$") )
...
redis:srem(bot_id.."Olifay:Removal:User:Group"..msg.chat_id_, text:match("^الغاء حظر (%d+)$") )
...
redis:sadd(bot_id.."Olifay:Silence:User:Group"..msg.chat_id_, text:match("^كتم (%d+)$"))
Send_Options(msg,text:match("^كتم (%d+)$"),"reply"," تم كتمه من هنا")  
end
end
if text and text:match("^الغاء كتم (%d+)$") and Admin(msg) then
redis:srem(bot_id.."Olifay:Silence:User:Group"..msg.chat_id_,text:match("^الغاء كتم (%d+)$") )
...
redis:set(bot_id.."Olifay:Lock:text"..msg.chat_id_,true) 
...
redis:set(bot_id.."Olifay:Lock:AddMempar"..msg.chat_id_,"kick")  
...
redis:set(bot_id.."Olifay:Lock:Join"..msg.chat_id_,"kick")  
...
redis:set(bot_id.."Olifay:Lock:Bot:kick"..msg.chat_id_,"del")  
...
redis:set(bot_id.."Olifay:Lock:Bot:kick"..msg.chat_id_,"kick")  
...
redis:set(bot_id.."Olifay:Lock:tagservr"..msg.chat_id_,true)  
...
redis:set(bot_id.."Olifay:lockpin"..msg.chat_id_, true) 
redis:sadd(bot_id.."Olifay:Lock:pin",msg.chat_id_) 
tdcli_function ({ ID = "GetChannelFull",  channel_id_ = msg.chat_id_:gsub("-100","") }, function(arg,data)  redis:set(bot_id.."Olifay:Get:Id:Msg:Pin"..msg.chat_id_,data.pinned_message_id_)  end,nil)
...
redis:set(bot_id.."Olifay:Lock:edit"..msg.chat_id_,true) 
...
redis:set(bot_id.."Olifay:Lock:tagservrbot"..msg.chat_id_,true)   
list ={"Lock:Bot:kick","Lock:User:Name","Lock:hashtak","Lock:Cmd","Lock:Link","Lock:forward","Lock:Keyboard","Lock:geam","Lock:Photo","Lock:Animation","Lock:Video","Lock:Audio","Lock:vico","Lock:Sticker","Lock:Document","Lock:Unsupported","Lock:Markdaun","Lock:Contact","Lock:Spam"}
...
redis:del(bot_id.."Olifay:Lock:AddMempar"..msg.chat_id_)  
...
redis:del(bot_id.."Olifay:Lock:text"..msg.chat_id_)  
...
redis:del(bot_id.."Olifay:Lock:Join"..msg.chat_id_)  
...
redis:del(bot_id.."Olifay:Lock:Bot:kick"..msg.chat_id_)  
...
redis:del(bot_id.."Olifay:Lock:tagservr"..msg.chat_id_)  
...
redis:del(bot_id.."Olifay:lockpin"..msg.chat_id_)  
redis:srem(bot_id.."Olifay:Lock:pin",msg.chat_id_)
...
redis:del(bot_id.."Olifay:Lock:edit"..msg.chat_id_) 
...
redis:del(bot_id.."Olifay:Lock:tagservrbot"..msg.chat_id_)   
list ={"Lock:Bot:kick","Lock:User:Name","Lock:hashtak","Lock:Cmd","Lock:Link","Lock:forward","Lock:Keyboard","Lock:geam","Lock:Photo","Lock:Animation","Lock:Video","Lock:Audio","Lock:vico","Lock:Sticker","Lock:Document","Lock:Unsupported","Lock:Markdaun","Lock:Contact","Lock:Spam"}
...
redis:set(bot_id.."Olifay:Link_Group"..msg.chat_id_,true) 
...
redis:del(bot_id.."Olifay:Lock:Ban:Group"..msg.chat_id_)
...
redis:set(bot_id.."Olifay:Cheking:Seted"..msg.chat_id_,"true")
...
redis:del(bot_id.."Olifay:Cheking:Seted"..msg.chat_id_)
send(msg.chat_id_, msg.id_, 'ܛ تم تفعيل رفع - ( الادمن - المميز ) ')
return false 
end
if text ==("تثبيت") and msg.reply_to_message_id_ ~= 0 and Admin(msg) then 
if redis:sismember(bot_id.."Olifay:Lock:pin",msg.chat_id_) and not Constructor(msg) then
...
redis:set(bot_id.."Olifay:Get:Id:Msg:Pin"..msg.chat_id_,msg.reply_to_message_id_)
return false 
end
...
redis:del(bot_id.."Olifay:Get:Id:Msg:Pin"..msg.chat_id_)
return false 
end
if data.code_ == 6 then
send(msg.chat_id_,msg.id_,"ܛ البوت ليس ادمن هنا")  
...
redis:del(bot_id.."Olifay:link:set:Group"..msg.chat_id_) 
return false 
end
...
redis:del(bot_id.."Olifay:Get:Welcome:Group"..msg.chat_id_) 
...
redis:del(bot_id.."Olifay::Rules:Group"..msg.chat_id_) 
return false 
end
if text == 'حذف الايدي' and Owner(msg) or text == 'مسح الايدي' and Owner(msg) then
redis:del(bot_id.."Olifay:Set:Id:Group"..msg.chat_id_)
send(msg.chat_id_, msg.id_, 'ܛ تم ازالة كليشة الايدي ')
return false 
end
if text == 'مسح رسائلي' then
redis:del(bot_id..'Olifay:Num:Message:User'..msg.chat_id_..':'..msg.sender_user_id_)
send(msg.chat_id_, msg.id_,'ܛ تم مسح جميع رسائلك ') 
return false 
end
if text == 'مسح سحكاتي' or text == 'مسح تعديلاتي' then
redis:del(bot_id..'Olifay:Num:Message:Edit'..msg.chat_id_..':'..msg.sender_user_id_)
send(msg.chat_id_, msg.id_,'ܛ تم مسح جميع تعديلاتك ') 
return false 
end
if text == 'مسح جهاتي' then
redis:del(bot_id..'Olifay:Num:Add:Memp'..msg.chat_id_..':'..msg.sender_user_id_)
...
redis:hset(bot_id.."Olifay:Spam:Group:User"..msg.chat_id_ ,"Num:Spam" ,text:match("^وضع تكرار (%d+)$")) 
...
redis:hset(bot_id.."Olifay:Spam:Group:User"..msg.chat_id_ ,"Num:Spam:Time" ,text:match("^وضع زمن التكرار (%d+)$")) 
...
redis:setex(bot_id.."Olifay:link:set"..msg.chat_id_..""..msg.sender_user_id_,120,true) 
return false 
end
...
redis:set(bot_id.."Olifay:Set:Chat:Photo"..msg.chat_id_..":"..msg.sender_user_id_,true) 
send(msg.chat_id_,msg.id_,"ܛ ارسل الصوره لوضعها") 
return false 
end
if text == "ضع وصف" and Admin(msg) or text == "وضع وصف" and Admin(msg) then  
redis:setex(bot_id.."Olifay:Change:Description" .. msg.chat_id_ .. "" .. msg.sender_user_id_, 120, true)  
send(msg.chat_id_,msg.id_,"ܛ ارسل الان الوصف")
return false 
end
if text == "ضع ترحيب" and Admin(msg) or text == "وضع ترحيب" and Admin(msg) then  
redis:setex(bot_id.."Olifay:Welcome:Group" .. msg.chat_id_ .. "" .. msg.sender_user_id_, 120, true)  
...
redis:setex(bot_id.."Olifay:Redis:Rules:" .. msg.chat_id_ .. ":" .. msg.sender_user_id_, 600, true) 
send(msg.chat_id_,msg.id_,"ܛ ارسل لي القوانين الان")  
return false 
end
if text == 'وضع كليشه المطور' and Dev_Olifay(msg) then
redis:set(bot_id..'Olifay:GetTexting:DevStorm'..msg.chat_id_..':'..msg.sender_user_id_,true)
send(msg.chat_id_,msg.id_,'ܛ  ارسل لي الكليشه الان')
return false 
end
...
if text == "الرابط" then 
local status_Link = redis:get(bot_id.."Olifay:Link_Group"..msg.chat_id_)
if not status_Link then
send(msg.chat_id_, msg.id_,"ܛ جلب الرابط معطل") 
return false  
end
local link = redis:get(bot_id.."Olifay:link:set:Group"..msg.chat_id_)            
if link then                              
...
if redis:get(bot_id.."Olifay:Get:Welcome:Group"..msg.chat_id_)   then 
Welcome = redis:get(bot_id.."Olifay:Get:Welcome:Group"..msg.chat_id_)  
else 
...
local list = redis:smembers(bot_id.."Olifay:List:Filter"..msg.chat_id_)  
for k,v in pairs(list) do  
redis:del(bot_id.."Olifay:Filter:Reply1"..msg.sender_user_id_..msg.chat_id_)  
redis:del(bot_id.."Olifay:Filter:Reply2"..v..msg.chat_id_)  
redis:srem(bot_id.."Olifay:List:Filter"..msg.chat_id_,v)  
end  
...
t = "\nܛ قائمة المنع \n━━━━━━━━━━━━━\n"
for k,v in pairs(list) do  
local FilterMsg = redis:get(bot_id.."Olifay:Filter:Reply2"..v..msg.chat_id_)   
t = t..""..k.."- "..v.." » {"..FilterMsg.."}\n"    
end  
if #list == 0 then  
...
redis:set(bot_id.."Olifay:Filter:Reply1"..msg.sender_user_id_..msg.chat_id_,"SetFilter")  
return false  
end
if text == "الغاء منع" and msg.reply_to_message_id_ == 0 and Admin(msg) then    
send(msg.chat_id_, msg.id_,"ܛ ارسل الكلمه الان")  
redis:set(bot_id.."Olifay:Filter:Reply1"..msg.sender_user_id_..msg.chat_id_,"DelFilter")  
return false 
end
...
local Set_Rules = redis:get(bot_id.."Olifay::Rules:Group" .. msg.chat_id_)   
if Set_Rules then     
...
redis:set(bot_id.."Olifay:Command:Reids:Group"..msg.chat_id_..":"..msg.sender_user_id_,"true") 
...
redis:set(bot_id.."Olifay:Command:Reids:Group:Del"..msg.chat_id_..":"..msg.sender_user_id_,"true") 
...
redis:del(bot_id.."Olifay:Add:Validity:Group:Rt"..ComdNew..msg.chat_id_)
redis:srem(bot_id.."Olifay:Validitys:Group"..msg.chat_id_,ComdNew)  
send(msg.chat_id_, msg.id_, "\nܛ تم مسح ← { "..ComdNew..' } من الصلاحيات') 
return false end
if text and text:match("^اضف صلاحيه (.*)$") and Admin(msg) then 
local ComdNew = text:match("^اضف صلاحيه (.*)$")
redis:set(bot_id.."Olifay:Add:Validity:Group:Rt:New"..msg.chat_id_..msg.sender_user_id_,ComdNew)  
redis:sadd(bot_id.."Olifay:Validitys:Group"..msg.chat_id_,ComdNew)  
redis:setex(bot_id.."Olifay:Redis:Validity:Group"..msg.chat_id_..""..msg.sender_user_id_,200,true)  
...
redis:set(bot_id.."Olifay:Sudo:User:Reply"..msg.chat_id_,Teext)
...
redis:set(bot_id.."Olifay:Basic:Constructor:Reply"..msg.chat_id_,Teext)
...
for k,v in pairs(list) do
redis:del(bot_id.."Olifay:Add:Rd:Manager:Gif"..v..msg.chat_id_)   
redis:del(bot_id.."Olifay:Add:Rd:Manager:Vico"..v..msg.chat_id_)   
redis:del(bot_id.."Olifay:Add:Rd:Manager:Stekrs"..v..msg.chat_id_)     
redis:del(bot_id.."Olifay:Add:Rd:Manager:Text"..v..msg.chat_id_)   
redis:del(bot_id.."Olifay:Add:Rd:Manager:Photo"..v..msg.chat_id_)
redis:del(bot_id.."Olifay:Add:Rd:Manager:Video"..v..msg.chat_id_)
redis:del(bot_id.."Olifay:Add:Rd:Manager:File"..v..msg.chat_id_)
redis:del(bot_id.."Olifay:Add:Rd:Manager:Audio"..v..msg.chat_id_)
redis:del(bot_id.."Olifay:List:Manager"..msg.chat_id_)
end
...
redis:set(bot_id.."Olifay:Set:Manager:rd"..msg.sender_user_id_..":"..msg.chat_id_,true)
return false end
if text == "حذف رد" and Owner(msg) then
send(msg.chat_id_, msg.id_,"ܛ ارسل الان الكلمه لحذفها من ردود المدير")
redis:set(bot_id.."Olifay:Set:Manager:rd"..msg.sender_user_id_..":"..msg.chat_id_,"true2")
return false end
if text == ("مسح ردود المطور") and Dev_Olifay(msg) then 
local list = redis:smembers(bot_id.."Olifay:List:Rd:Sudo")
for k,v in pairs(list) do
redis:del(bot_id.."Olifay:Add:Rd:Sudo:Gif"..v)   
redis:del(bot_id.."Olifay:Add:Rd:Sudo:vico"..v)   
redis:del(bot_id.."Olifay:Add:Rd:Sudo:stekr"..v)     
redis:del(bot_id.."Olifay:Add:Rd:Sudo:Text"..v)   
redis:del(bot_id.."Olifay:Add:Rd:Sudo:Photo"..v)
redis:del(bot_id.."Olifay:Add:Rd:Sudo:Video"..v)
redis:del(bot_id.."Olifay:Add:Rd:Sudo:File"..v)
redis:del(bot_id.."Olifay:Add:Rd:Sudo:Audio"..v)
redis:del(bot_id.."Olifay:List:Rd:Sudo")
end
send(msg.chat_id_, msg.id_,"ܛ تم حذف ردود المطور")
return false end
if text == ("ردود المطور") and Dev_Olifay(msg) then 
local list = redis:smembers(bot_id.."Olifay:List:Rd:Sudo")
text = "\nܛ قائمة ردود المطور \n━━━━━━━━━━━━━\n"
for k,v in pairs(list) do
if redis:get(bot_id.."Olifay:Add:Rd:Sudo:Gif"..v) then
db = "متحركه 🎭"
elseif redis:get(bot_id.."Olifay:Add:Rd:Sudo:vico"..v) then
db = "بصمه 📢"
elseif redis:get(bot_id.."Olifay:Add:Rd:Sudo:stekr"..v) then
db = "ملصق 🃏"
elseif redis:get(bot_id.."Olifay:Add:Rd:Sudo:Text"..v) then
db = "رساله ✉"
elseif redis:get(bot_id.."Olifay:Add:Rd:Sudo:Photo"..v) then
db = "صوره 🎇"
elseif redis:get(bot_id.."Olifay:Add:Rd:Sudo:Video"..v) then
db = "فيديو 📹"
elseif redis:get(bot_id.."Olifay:Add:Rd:Sudo:File"..v) then
db = "ملف 📁"
elseif redis:get(bot_id.."Olifay:Add:Rd:Sudo:Audio"..v) then
db = "اغنيه 🎵"
end
text = text..""..k.." » {"..v.."} » {"..db.."}\n"
end
...
send(msg.chat_id_, msg.id_,"["..text.."]")
return false end
if text == "اضف رد للكل" and Dev_Olifay(msg) then 
send(msg.chat_id_, msg.id_,"ܛ ارسل الان الكلمه لاضافتها في ردود المكور ")
redis:set(bot_id.."Olifay:Set:Rd"..msg.sender_user_id_..":"..msg.chat_id_,true)
return false end
if text == "حذف رد للكل" and Dev_Olifay(msg) then 
send(msg.chat_id_, msg.id_,"ܛ ارسل الان الكلمه لحذفها من ردود المطور")
redis:set(bot_id.."Olifay:Set:On"..msg.sender_user_id_..":"..msg.chat_id_,true)
end
...
if redis:sismember(bot_id.."Olifay:Sudo:User",result.id_) then
dev = "المطور ،" else dev = "" end
if redis:sismember(bot_id.."Olifay:Basic:Constructor"..msg.chat_id_, result.id_) then
crr = "منشئ اساسي ،" else crr = "" end
if redis:sismember(bot_id..'Olifay:Constructor'..msg.chat_id_, result.id_) then
cr = "منشئ ،" else cr = "" end
if redis:sismember(bot_id..'Olifay:Manager'..msg.chat_id_, result.id_) then
own = "مدير ،" else own = "" end
if redis:sismember(bot_id..'Olifay:Mod:User'..msg.chat_id_, result.id_) then
mod = "ادمن ،" else mod = "" end
if redis:sismember(bot_id..'Olifay:Special:User'..msg.chat_id_, result.id_) then
vip = "مميز ،" else vip = ""
end
...
if Dev_Olifay_User(msg.sender_user_id_) == true then
redis:srem(bot_id.."Olifay:Sudo:User", result.id_)
redis:srem(bot_id.."Olifay:Basic:Constructor"..msg.chat_id_,result.id_)
redis:srem(bot_id..'Olifay:Constructor'..msg.chat_id_, result.id_)
redis:srem(bot_id..'Olifay:Manager'..msg.chat_id_, result.id_)
redis:srem(bot_id..'Olifay:Mod:User'..msg.chat_id_, result.id_)
redis:srem(bot_id..'Olifay:Special:User'..msg.chat_id_, result.id_)
elseif redis:sismember(bot_id.."Olifay:Sudo:User",msg.sender_user_id_) then
redis:srem(bot_id..'Olifay:Mod:User'..msg.chat_id_, result.id_)
redis:srem(bot_id..'Olifay:Special:User'..msg.chat_id_, result.id_)
redis:srem(bot_id..'Olifay:Manager'..msg.chat_id_, result.id_)
redis:srem(bot_id..'Olifay:Constructor'..msg.chat_id_, result.id_)
redis:srem(bot_id.."Olifay:Basic:Constructor"..msg.chat_id_,result.id_)
elseif redis:sismember(bot_id.."Olifay:Basic:Constructor"..msg.chat_id_, msg.sender_user_id_) then
redis:srem(bot_id..'Olifay:Mod:User'..msg.chat_id_, result.id_)
redis:srem(bot_id..'Olifay:Special:User'..msg.chat_id_, result.id_)
redis:srem(bot_id..'Olifay:Manager'..msg.chat_id_, result.id_)
redis:srem(bot_id..'Olifay:Constructor'..msg.chat_id_, result.id_)
elseif redis:sismember(bot_id..'Olifay:Constructor'..msg.chat_id_, msg.sender_user_id_) then
redis:srem(bot_id..'Olifay:Mod:User'..msg.chat_id_, result.id_)
redis:srem(bot_id..'Olifay:Special:User'..msg.chat_id_, result.id_)
redis:srem(bot_id..'Olifay:Manager'..msg.chat_id_, result.id_)
elseif redis:sismember(bot_id..'Olifay:Manager'..msg.chat_id_, msg.sender_user_id_) then
redis:srem(bot_id..'Olifay:Mod:User'..msg.chat_id_, result.id_)
redis:srem(bot_id..'Olifay:Special:User'..msg.chat_id_, result.id_)
end
end
...
if redis:sismember(bot_id.."Olifay:Sudo:User",result.sender_user_id_) then
dev = "المطور ،" else dev = "" end
if redis:sismember(bot_id.."Olifay:Basic:Constructor"..msg.chat_id_, result.sender_user_id_) then
crr = "منشئ اساسي ،" else crr = "" end
if redis:sismember(bot_id..'Olifay:Constructor'..msg.chat_id_, result.sender_user_id_) then
cr = "منشئ ،" else cr = "" end
if redis:sismember(bot_id..'Olifay:Manager'..msg.chat_id_, result.sender_user_id_) then
own = "مدير ،" else own = "" end
if redis:sismember(bot_id..'Olifay:Mod:User'..msg.chat_id_, result.sender_user_id_) then
mod = "ادمن ،" else mod = "" end
if redis:sismember(bot_id..'Olifay:Special:User'..msg.chat_id_, result.sender_user_id_) then
vip = "مميز ،" else vip = ""
end
...
if text == "غادر" then 
if DeveloperBot(msg) and not redis:get(bot_id.."Olifay:Lock:Left"..msg.chat_id_) then 
...
redis:srem(bot_id.."Olifay:Chek:Groups",GP_ID[2])  
end
end
if text == (redis:get(bot_id.."Olifay:Redis:Name:Bot") or "اولفاي") then
local namebot = {
"عمر "..(redis:get(bot_id.."Olifay:Redis:Name:Bot") or "اولفاي").. " شتريد؟",
"أჂ̤ أჂ̤ هياتني اني",
"موجود بس لتصيح",
"لتــلح دا احجي ويه بنات سكر بعدين اجاوبك",
"راح نموت بكورونا ونته بعدك تصيح "..(redis:get(bot_id.."Olifay:Redis:Name:Bot") or "اولفاي"),
'يمعود والله نعسان'
}
...
if text == "بوت" then
local BotName = {
"باوع لك خليني احبك وصيحلي باسمي "..(redis:get(bot_id.."Olifay:Redis:Name:Bot") or "اولفاي").. "",
"اهووو هم ناداني ذا لبزر ياخي وش تبي",
"لح لح",
"ها يازق",
"باوع لك خليني احبك وصيحلي باسمي "..(redis:get(bot_id.."Olifay:Redis:Name:Bot") or "اولفاي").. "",
"راح نموت بكورونا ونته بعدك تصيح بوت يابه اسمي "..(redis:get(bot_id.."Olifay:Redis:Name:Bot") or "اولفاي").. ""
}
BotNameText = math.random(#BotName)
send(msg.chat_id_, msg.id_,BotName[BotNameText]) 
end
if text == "تغير اسم البوت" and Dev_Olifay(msg) or text == "تغيير اسم البوت" and Dev_Olifay(msg) then 
redis:setex(bot_id.."Olifay:Change:Name:Bot"..msg.sender_user_id_,300,true) 
send(msg.chat_id_, msg.id_,"ܛ  ارسل لي الاسم الان ")  
end
if text=="اذاعه خاص" and msg.reply_to_message_id_ == 0 and DeveloperBot(msg) then 
if redis:get(bot_id.."Olifay:Broadcasting:Bot") and not Dev_Olifay(msg) then 
...
if redis:get(bot_id.."Olifay:lockpin"..msg.chat_id_) then    
lock_pin = "{✔️}"
else 
lock_pin = "{✖}"    
end
if redis:get(bot_id.."Olifay:Lock:tagservr"..msg.chat_id_) then    
lock_tagservr = "{✔️}"
else 
lock_tagservr = "{✖}"
end
if redis:get(bot_id.."Olifay:Lock:text"..msg.chat_id_) then    
lock_text = "← {✔️}"
else 
lock_text = "← {✖}"    
end
...
if redis:get(bot_id.."Olifay:Lock:edit"..msg.chat_id_) then    
lock_edit = "← {✔️}"
else 
lock_edit = "← {✖}"    
end
if redis:get(bot_id.."Olifay:Chek:Welcome"..msg.chat_id_) then
welcome = "← {✔️}"
else 
welcome = "← {✖}"    
end
if redis:hget(bot_id.."Olifay:Spam:Group:User"..msg.chat_id_, "Spam:User") == "kick" then     
flood = "← { بالطرد }"     
elseif redis:hget(bot_id.."Olifay:Spam:Group:User"..msg.chat_id_,"Spam:User") == "keed" then     
flood = "← { بالتقيد }"     
elseif redis:hget(bot_id.."Olifay:Spam:Group:User"..msg.chat_id_,"Spam:User") == "mute" then     
flood = "← { بالكتم }"           
elseif redis:hget(bot_id.."Olifay:Spam:Group:User"..msg.chat_id_,"Spam:User") == "del" then     
flood = "← {✔️}"
else     
flood = "← {✖}"     
end
...
if redis:get(bot_id.."Olifay:Lock:Link"..msg.chat_id_) == "del" then
lock_links = "← {✔️}"
elseif redis:get(bot_id.."Olifay:Lock:Link"..msg.chat_id_) == "ked" then
lock_links = "← { بالتقيد }"    
elseif redis:get(bot_id.."Olifay:Lock:Link"..msg.chat_id_) == "ktm" then
lock_links = "← { بالكتم }"    
elseif redis:get(bot_id.."Olifay:Lock:Link"..msg.chat_id_) == "kick" then
lock_links = "← { بالطرد }"    
else
lock_links = "← {✖}"    
end
if redis:get(bot_id.."Olifay:Lock:Cmd"..msg.chat_id_) == "del" then
lock_cmds = "← {✔️}"
elseif redis:get(bot_id.."Olifay:Lock:Cmd"..msg.chat_id_) == "ked" then
lock_cmds = "← { بالتقيد }"    
elseif redis:get(bot_id.."Olifay:Lock:Cmd"..msg.chat_id_) == "ktm" then
lock_cmds = "← { بالكتم }"   
elseif redis:get(bot_id.."Olifay:Lock:Cmd"..msg.chat_id_) == "kick" then
lock_cmds = "← { بالطرد }"    
else
lock_cmds = "← {✖}"    
end
if redis:get(bot_id.."Olifay:Lock:User:Name"..msg.chat_id_) == "del" then
lock_user = "← {✔️}"
elseif redis:get(bot_id.."Olifay:Lock:User:Name"..msg.chat_id_) == "ked" then
lock_user = "← { بالتقيد }"    
elseif redis:get(bot_id.."Olifay:Lock:User:Name"..msg.chat_id_) == "ktm" then
lock_user = "← { بالكتم }"    
elseif redis:get(bot_id.."Olifay:Lock:User:Name"..msg.chat_id_) == "kick" then
lock_user = "← { بالطرد }"    
else
lock_user = "← {✖}"    
end
if redis:get(bot_id.."Olifay:Lock:hashtak"..msg.chat_id_) == "del" then
lock_hash = "← {✔️}"
elseif redis:get(bot_id.."Olifay:Lock:hashtak"..msg.chat_id_) == "ked" then 
lock_hash = "← { بالتقيد }"    
elseif redis:get(bot_id.."Olifay:Lock:hashtak"..msg.chat_id_) == "ktm" then 
lock_hash = "← { بالكتم }"    
elseif redis:get(bot_id.."Olifay:Lock:hashtak"..msg.chat_id_) == "kick" then 
lock_hash = "← { بالطرد }"    
else
lock_hash = "← {✖}"    
end
...
if redis:get(bot_id.."Olifay:Lock:Bot:kick"..msg.chat_id_) == "del" then
lock_bots = "← {✔️}"
elseif redis:get(bot_id.."Olifay:Lock:Bot:kick"..msg.chat_id_) == "ked" then
lock_bots = "← { بالتقيد }"   
elseif redis:get(bot_id.."Olifay:Lock:Bot:kick"..msg.chat_id_) == "kick" then
lock_bots = "← { بالطرد }"    
else
lock_bots = "← {✖}"    
end
...
if redis:get(bot_id.."Olifay:Lock:Spam"..msg.chat_id_) == "del" then    
lock_spam = "← {✔️}"
elseif redis:get(bot_id.."Olifay:Lock:Spam"..msg.chat_id_) == "ked" then 
lock_spam = "← { بالتقيد }"    
elseif redis:get(bot_id.."Olifay:Lock:Spam"..msg.chat_id_) == "ktm" then 
lock_spam = "← { بالكتم }"    
elseif redis:get(bot_id.."Olifay:Lock:Spam"..msg.chat_id_) == "kick" then 
lock_spam = "← { بالطرد }"    
else
lock_spam = "← {✖}"    
end        
if not redis:get(bot_id.."Olifay:Reply:Manager"..msg.chat_id_) then
ReplyManager = "← {✔️}"
else
ReplyManager = "← {✖}"
end
if not redis:get(bot_id.."Olifay:Reply:Sudo"..msg.chat_id_) then
ReplySudo = "← {✔️}"
else
ReplySudo = "← {✖}"
end
if not redis:get(bot_id.."Olifay:Lock:Id:Photo"..msg.chat_id_)  then
IdPhoto = "← {✔️}"
else
IdPhoto = "← {✖}"
end
if not redis:get(bot_id.."Olifay:Lock:Id:Py:Photo"..msg.chat_id_) then
IdPyPhoto = "← {✔️}"
else
IdPyPhoto = "← {✖}"
end
if not redis:get(bot_id.."Olifay:Cheking:Kick:Me:Group"..msg.chat_id_)  then
KickMe = "← {✔️}"
else
KickMe = "← {✖}"
end
if not redis:get(bot_id.."Olifay:Lock:Ban:Group"..msg.chat_id_)  then
Banusers = "← {✔️}"
else
Banusers = "← {✖}"
end
if not redis:get(bot_id.."Olifay:Cheking:Seted"..msg.chat_id_) then
Setusers = "← {✔️}"
else
Setusers = "← {✖}"
end
if redis:get(bot_id.."Olifay:Link_Group"..msg.chat_id_) then
Link_Group = "← {✔️}"
else
Link_Group = "← {✖}"
end
if not redis:get(bot_id.."Olifay:Fun:Group"..msg.chat_id_) then
FunGroup = "← {✔️}"
else
FunGroup = "← {✖}"
end
local Num_Flood = redis:hget(bot_id.."Olifay:Spam:Group:User"..msg.chat_id_,"Num:Spam") or 0
send(msg.chat_id_, msg.id_,"*\nܛ اعدادات المجموعه "..
"\n━━━━━━━━━━━━━"..
...
redis:setex(bot_id.."Olifay:Redis:Id:Group"..msg.chat_id_..""..msg.sender_user_id_,240,true)  
send(msg.chat_id_, msg.id_,[[
ܛ ارسل الان النص
ܛ يمكنك اضافه :
...
if text == 'ايدي' and tonumber(msg.reply_to_message_id_) == 0 and not redis:get(bot_id..'Olifay:Lock:Id:Photo'..msg.chat_id_) then
...
local NumMessageEdit = redis:get(bot_id..'Olifay:Num:Message:Edit'..msg.chat_id_..msg.sender_user_id_) or 0
local Num_Games = redis:get(bot_id.."Olifay:Num:Add:Games"..msg.chat_id_..msg.sender_user_id_) or 0
local Add_Mem = redis:get(bot_id.."Olifay:Num:Add:Memp"..msg.chat_id_..":"..msg.sender_user_id_) or 0
...
local Get_Is_Id = redis:get(bot_id.."Olifay:Set:Id:Group"..msg.chat_id_)
if not redis:get(bot_id..'Olifay:Lock:Id:Py:Photo'..msg.chat_id_) then
...
local Id = data.id_
local NumMsg = redis:get(bot_id..'Olifay:Num:Message:User'..msg.chat_id_..':'..data.id_) or 0
local TotalMsg = Total_message(NumMsg)
local Status_Gps = Get_Rank(Id,msg.chat_id_)
local NumMessageEdit = redis:get(bot_id..'Olifay:Num:Message:Edit'..msg.chat_id_..data.id_) or 0
local Num_Games = redis:get(bot_id.."Olifay:Msg_User"..msg.chat_id_..":"..data.id_) or 0
local Add_Mem = redis:get(bot_id.."Olifay:Num:Add:Memp"..msg.chat_id_..":"..data.id_) or 0
send(msg.chat_id_, msg.id_,'\n* 𝒊𝒅 𓂅  '..Id..' 𓍯\n 𝒎𝒔𝒈𝒆𓂅 '..NumMsg..' 𓍯\n 𝒖𝒔𝒆𝒓𓂅 *['..UserName_User..'] 𓍯*\n 𝒔𝒕𝒂𓂅 '..Status_Gps..' 𓍯*') 
end,nil)   
end
tdcli_function ({ID = "GetMessage",chat_id_ = msg.chat_id_,message_id_ = tonumber(msg.reply_to_message_id_)}, Function_Status, nil)
...
if text and text:match("^ايدي @(.*)$") and not redis:get(bot_id..'Olifay:Lock:Id:Photo'..msg.chat_id_) then
...
if redis:get(bot_id.."Olifay:Lock:Game:Group"..msg.chat_id_) then
redis:del(bot_id.."Olifay:Set:Sma"..msg.chat_id_)
Random = {"🍏","🍎","🍐","🍊","🍋","🍉","🍇","🍓","🍈","🍒","🍑","🍍","🥥","🥝","🍅","🍆","🥑","🥦","🥒","🌶","🌽","🥕","🥔","🥖","🥐","🍞","🥨","🍟","🧀","🥚","🍳","🥓","🥩","🍗","🍖","🌭","🍔","🍠","🍕","🥪","🥙","☕️","??","🥤","🍶","🍺","🍻","🏀","⚽️","🏈","⚾️","🎾","🏐","🏉","🎱","🏓","🏸","🥅","🎰","🎮","🎳","🎯","🎲","🎻","🎸","🎺","🥁","🎹","🎼","🎧","🎤","🎬","🎨","🎭","🎪","🎟","🎫","🎗","🏵","🎖","🏆","🥌","🛷","🚗","🚌","🏎","🚓","🚑","🚚","🚛","🚜","🇮🇶","⚔","🛡","🔮","🌡","💣","📌","📍","📓","📗","📂","📅","📪","📫","📬","📭","⏰","📺","🎚","☎️","📡"}
SM = Random[math.random(#Random)]
redis:set(bot_id.."Olifay:Random:Sm"..msg.chat_id_,SM)
...
if redis:get(bot_id.."Olifay:Lock:Game:Group"..msg.chat_id_) then
redis:del(bot_id.."Olifay:Speed:Tr"..msg.chat_id_)
KlamSpeed = {"سحور","سياره","استقبال","قنفه","ايفون","بزونه","مطبخ","كرستيانو","دجاجه","مدرسه","الوان","غرفه","ثلاجه","كهوه","سفينه","العراق","محطه","طياره","رادار","منزل","مستشفى","كهرباء","تفاحه","اخطبوط","سلمون","فرنسا","برتقاله","تفاح","مطرقه","بتيته","لهانه","شباك","باص","سمكه","ذباب","تلفاز","حاسوب","انترنيت","ساحه","جسر"};
name = KlamSpeed[math.random(#KlamSpeed)]
redis:set(bot_id.."Olifay:Klam:Speed"..msg.chat_id_,name)
...
if text == "حزوره" then
if redis:get(bot_id.."Olifay:Lock:Game:Group"..msg.chat_id_) then
redis:del(bot_id.."Olifay:Set:Hzora"..msg.chat_id_)
Hzora = {"الجرس","عقرب الساعه","السمك","المطر","5","الكتاب","البسمار","7","الكعبه","بيت الشعر","لهانه","انا","امي","الابره","الساعه","22","غلط","كم الساعه","البيتنجان","البيض","المرايه","الضوء","الهواء","الضل","العمر","القلم","المشط","الحفره","البحر","الثلج","الاسفنج","الصوت","بلم"};
name = Hzora[math.random(#Hzora)]
redis:set(bot_id.."Olifay:Klam:Hzor"..msg.chat_id_,name)
...
if text == "معاني" then
if redis:get(bot_id.."Olifay:Lock:Game:Group"..msg.chat_id_) then
redis:del(bot_id.."Olifay:Set:Maany"..msg.chat_id_)
Maany_Rand = {"قرد","دجاجه","بطريق","ضفدع","بومه","نحله","ديك","جمل","بقره","دولفين","تمساح","قرش","نمر","اخطبوط","سمكه","خفاش","اسد","فأر","ذئب","فراشه","عقرب","زرافه","قنفذ","تفاحه","باذنجان"}
name = Maany_Rand[math.random(#Maany_Rand)]
redis:set(bot_id.."Olifay:Maany"..msg.chat_id_,name)
...
if text == "العكس" then
if redis:get(bot_id.."Olifay:Lock:Game:Group"..msg.chat_id_) then
redis:del(bot_id.."Olifay:Set:Aks"..msg.chat_id_)
katu = {"باي","فهمت","موزين","اسمعك","احبك","موحلو","نضيف","حاره","ناصي","جوه","سريع","ونسه","طويل","سمين","ضعيف","شريف","شجاع","رحت","عدل","نشيط","شبعان","موعطشان","خوش ولد","اني","هادئ"}
name = katu[math.random(#katu)]
redis:set(bot_id.."Olifay:Set:Aks:Game"..msg.chat_id_,name)
...
redis:set(bot_id.."Olifay:GAMES:NUM"..msg.chat_id_,Num) 
...
redis:setex(bot_id.."Olifay:GAME:TKMEN" .. msg.chat_id_ .. "" .. msg.sender_user_id_, 100, true)  
return false  
end
end
...
if redis:get(bot_id.."Olifay:Lock:Game:Group"..msg.chat_id_) then   
Num = math.random(1,6)
redis:set(bot_id.."Olifay:Games:Bat"..msg.chat_id_,Num) 
send(msg.chat_id_, msg.id_,[[
*➀       ➁     ➂      ➃      ➄     ➅
↓      ↓     ↓      ↓     ↓     ↓
👊 ‹› 👊 ‹› 👊 ‹› 👊 ‹› 👊 ‹› 👊
...
redis:setex(bot_id.."Olifay:SET:GAME" .. msg.chat_id_ .. "" .. msg.sender_user_id_, 100, true)  
return false  
end
end
...
redis:del(bot_id.."Olifay:Set:Moktlf:Bot"..msg.chat_id_)
redis:set(bot_id.."Olifay::Set:Moktlf"..msg.chat_id_,name)
...
redis:set(bot_id.."Olifay:Set:Amth"..msg.chat_id_,name)
redis:del(bot_id.."Olifay:Set:Amth:Bot"..msg.chat_id_)
...
if text == 'رسائلي' then
local nummsg = redis:get(bot_id..'Olifay:Num:Message:User'..msg.chat_id_..':'..msg.sender_user_id_) or 1
send(msg.chat_id_, msg.id_,'ܛ عدد رسائلك هنا *~ '..nummsg..'*') 
end
if text == 'سحكاتي' or text == 'تعديلاتي' then
local edit = redis:get(bot_id..'Olifay:Num:Message:Edit'..msg.chat_id_..msg.sender_user_id_) or 0
send(msg.chat_id_, msg.id_,'ܛ عدد التعديلات هنا *~ '..edit..'*') 
end
if text == 'جهاتي' then
local addmem = redis:get(bot_id.."Olifay:Num:Add:Memp"..msg.chat_id_..":"..msg.sender_user_id_) or 0
send(msg.chat_id_, msg.id_,'ܛ عدد جهاتك المضافه هنا *~ '..addmem..'*') 
end
if text == "مجوهراتي" then 
local Num = redis:get(bot_id.."Olifay:Num:Add:Games"..msg.chat_id_..msg.sender_user_id_) or 0
if Num == 0 then 
...
if tonumber(redis:get(bot_id.."Olifay:Num:Add:Games"..msg.chat_id_..msg.sender_user_id_)) == tonumber(0) then
send(msg.chat_id_,msg.id_,"ܛ ليس لديك جواهر من الالعاب \nܛ اذا كنت تريد ربح الجواهر \nܛ ارسل الالعاب وابدأ اللعب ! ") 
else
local NUM_GAMES = redis:get(bot_id.."Olifay:Num:Add:Games"..msg.chat_id_..msg.sender_user_id_)
if tonumber(NUMPY) > tonumber(NUM_GAMES) then
send(msg.chat_id_,msg.id_,"\nܛ ليس لديك جواهر بهاذا العدد \nܛ لزيادة مجوهراتك في اللعبه \nܛ ارسل الالعاب وابدأ اللعب !") 
return false 
end
local NUMNKO = (NUMPY * 50)
redis:decrby(bot_id.."Olifay:Num:Add:Games"..msg.chat_id_..msg.sender_user_id_,NUMPY)  
redis:incrby(bot_id.."Olifay:Num:Message:User"..msg.chat_id_..":"..msg.sender_user_id_,NUMNKO)  
...
redis:set(bot_id.."Olifay:id:user"..msg.chat_id_,taha)  
redis:setex(bot_id.."Olifay:Add:msg:user" .. msg.chat_id_ .. "" .. msg.sender_user_id_, 120, true)  
...
redis:set(bot_id.."Olifay:idgem:user"..msg.chat_id_,taha)  
redis:setex(bot_id.."Olifay:gemadd:user" .. msg.chat_id_ .. "" .. msg.sender_user_id_, 120, true)  
send(msg.chat_id_, msg.id_, "ܛ ارسل لي عدد المجوهرات الان") 
end
if text and text:match("^اضف مجوهرات (%d+)$") and msg.reply_to_message_id_ ~= 0 and Constructor(msg) then
function reply(extra, result, success)
redis:incrby(bot_id.."Olifay:Num:Add:Games"..msg.chat_id_..result.sender_user_id_,text:match("^اضف مجوهرات (%d+)$"))  
...
if text and text:match("^اضف رسائل (%d+)$") and msg.reply_to_message_id_ ~= 0 and Constructor(msg) then
function reply(extra, result, success)
redis:del(bot_id.."Olifay:Msg_User"..msg.chat_id_..":"..result.sender_user_id_) 
redis:incrby(bot_id.."Olifay:Num:Message:User"..msg.chat_id_..":"..result.sender_user_id_,text:match("^اضف رسائل (%d+)$"))  
send(msg.chat_id_, msg.id_, "ܛ تم اضافه عدد الرسائل : "..text:match("^اضف رسائل (%d+)$").." ")  
end
tdcli_function ({ID = "GetMessage",chat_id_=msg.chat_id_,message_id_=tonumber(msg.reply_to_message_id_)},reply, nil)
return false
end
if text == "تنظيف المشتركين" and Dev_Olifay(msg) then
local pv = redis:smembers(bot_id..'Olifay:UsersBot')  
local sendok = 0
for i = 1, #pv do
tdcli_function({ID='GetChat',chat_id_ = pv[i]},function(arg,dataq)
...
if text == "تنظيف الكروبات" and Dev_Olifay(msg) then
local group = redis:smembers(bot_id..'Olifay:Chek:Groups')  
local w = 0
local q = 0
for i = 1, #group do
...
function Function_Status(extra, result, success)
if result.id_ then
if redis:sismember(bot_id.."Olifay:Silence:User:Group"..msg.chat_id_,result.id_) then
Muted = "مكتوم"
else
Muted = "غير مكتوم"
end
if redis:sismember(bot_id.."Olifay:Removal:User:Group"..msg.chat_id_,result.id_) then
Ban = "محظور"
else
Ban = "غير محظور"
end
if redis:sismember(bot_id.."Olifay:Removal:User:Groups",result.id_) then
GBan = "محظور عام"
else
GBan = "غير محظور عام"
end
...
function Function_Status(extra, result, success)
if redis:sismember(bot_id.."Olifay:Silence:User:Group"..msg.chat_id_,result.sender_user_id_) then
Muted = "مكتوم"
else
Muted = "غير مكتوم"
end
if redis:sismember(bot_id.."Olifay:Removal:User:Group"..msg.chat_id_,result.sender_user_id_) then
Ban = "محظور"
else
Ban = "غير محظور"
end
if redis:sismember(bot_id.."Olifay:Removal:User:Groups",result.sender_user_id_) then
GBan = "محظور عام"
else
GBan = "غير محظور عام"
end
...
redis:sadd(bot_id..'Olifay:Mod:User'..msg.chat_id_, admins[i].user_id_)
num2 = num2 + 1
tdcli_function ({ID = "GetUser",user_id_ = admins[i].user_id_},function(arg,b) 
if b.username_ == true then
end
if b.first_name_ == false then
redis:srem(bot_id..'Olifay:Mod:User'..msg.chat_id_, admins[i].user_id_)
end
end,nil)   
else
redis:sadd(bot_id..'Olifay:Mod:User'..msg.chat_id_, admins[i].user_id_)
...
local UserName = (b.username_ or "Olifay")
...
redis:sadd(bot_id.."Olifay:Basic:Constructor"..msg.chat_id_,b.id_)
end,nil)   
end,nil)   
end
if text and text:match("^تعين عدد الاعضاء (%d+)$") and Dev_Olifay(msg) then
redis:set(bot_id..'Olifay:Num:Add:Bot',text:match("تعين عدد الاعضاء (%d+)$") ) 
send(msg.chat_id_, msg.id_,'*ܛ  تم تعيين عدد اعضاء تفعيل البوت اكثر من : '..text:match("تعين عدد الاعضاء (%d+)$")..' عضو *')
end
...
local TextingDevStorm = redis:get(bot_id..'Olifay:Texting:DevStorm')
...
if text == 'حذف كليشه المطور' and Dev_Olifay(msg) then
redis:del(bot_id..'Olifay:Texting:DevStorm')
send(msg.chat_id_, msg.id_,'ܛ  تم حذف كليشه المطور')
end
end
...
redis:srem(bot_id..'Olifay:Chek:Groups','-100'..data.channel_.id_)  
end
...
redis:set(bot_id..'Olifay:Save:Username'..msg.sender_user_id_,data.username_)
end;end,nil)   
--------------------------------------------------------------------------------------------------------------
if text and redis:get(bot_id.."Olifay:Command:Reids:Group:Del"..msg.chat_id_..":"..msg.sender_user_id_) == "true" then
local NewCmmd = redis:get(bot_id.."Olifay:Get:Reides:Commands:Group"..msg.chat_id_..":"..text)
if NewCmmd then
redis:del(bot_id.."Olifay:Get:Reides:Commands:Group"..msg.chat_id_..":"..text)
redis:del(bot_id.."Olifay:Command:Reids:Group:New"..msg.chat_id_)
redis:srem(bot_id.."Olifay:Command:List:Group"..msg.chat_id_,text)
send(msg.chat_id_, msg.id_,"ܛ تم ازالة هاذا ← { "..text.." }")  
else
send(msg.chat_id_, msg.id_,"ܛ لا يوجد امر بهاذا الاسم")  
end
redis:del(bot_id.."Olifay:Command:Reids:Group:Del"..msg.chat_id_..":"..msg.sender_user_id_)
return false
end
------------------------------------------------------------------------------------------------------------
if text and redis:get(bot_id.."Olifay:Command:Reids:Group"..msg.chat_id_..":"..msg.sender_user_id_) == "true" then
redis:set(bot_id.."Olifay:Command:Reids:Group:New"..msg.chat_id_,text)
send(msg.chat_id_, msg.id_,"ܛ ارسل الامر الجديد ليتم وضعه مكان القديم")  
redis:del(bot_id.."Olifay:Command:Reids:Group"..msg.chat_id_..":"..msg.sender_user_id_)
redis:set(bot_id.."Olifay:Command:Reids:Group:End"..msg.chat_id_..":"..msg.sender_user_id_,"true1") 
return false
end
------------------------------------------------------------------------------------------------------------
if text and redis:get(bot_id.."Olifay:Command:Reids:Group:End"..msg.chat_id_..":"..msg.sender_user_id_) == "true1" then
local NewCmd = redis:get(bot_id.."Olifay:Command:Reids:Group:New"..msg.chat_id_)
redis:set(bot_id.."Olifay:Get:Reides:Commands:Group"..msg.chat_id_..":"..text,NewCmd)
redis:sadd(bot_id.."Olifay:Command:List:Group"..msg.chat_id_,text)
send(msg.chat_id_, msg.id_,"ܛ تم حفظ الامر باسم ← { "..text..' }')  
redis:del(bot_id.."Olifay:Command:Reids:Group:End"..msg.chat_id_..":"..msg.sender_user_id_)
return false
end
------------------------------------------------------------------------------------------------------------
...
elseif text and not redis:sismember(bot_id..'Olifay:Spam_For_Bot'..msg.sender_user_id_,text) then
redis:del(bot_id..'Olifay:Spam_For_Bot'..msg.sender_user_id_) 
elseif msg.content_.ID == "MessageChatAddMembers" then  
redis:set(bot_id.."Olifay:Who:Added:Me"..msg.chat_id_..":"..msg.content_.members_[0].id_,msg.sender_user_id_)
local mem_id = msg.content_.members_  
local Bots = redis:get(bot_id.."Olifay:Lock:Bot:kick"..msg.chat_id_) 
for i=0,#mem_id do  
...
if text and text:match('^'..(redis:get(bot_id.."Olifay:Redis:Name:Bot") or "اولفاي")..' ') then
data.message_.content_.text_ = data.message_.content_.text_:gsub('^'..(redis:get(bot_id.."Olifay:Redis:Name:Bot") or "اولفاي")..' ','')
end
if text then
local NewCmmd = redis:get(bot_id.."Olifay:Get:Reides:Commands:Group"..msg.chat_id_..":"..data.message_.content_.text_)
if NewCmmd then
data.message_.content_.text_ = (NewCmmd or data.message_.content_.text_)
end
end
--------------------------------------------------------------------------------------------------------------
Dev_Olifay_File(msg,data)
FilesOlifayBot(msg)
...
redis:incr(bot_id..'Olifay:Num:Message:Edit'..result.chat_id_..result.sender_user_id_)
if redis:get(bot_id.."Olifay:Lock:edit"..msg.chat_id_) and not textedit and not PresidentGroup(result) then
...
if redis:get(bot_id.."Olifay:Lock:Link"..msg.chat_id_) then
Delete_Message(result.chat_id_,{[0] = data.message_id_}) 
return false
end 
...
if redis:get(bot_id.."Olifay:Lock:User:Name"..msg.chat_id_) then
Delete_Message(result.chat_id_,{[0] = data.message_id_}) 
return false
end  
elseif textedit and textedit:match("@") then
...
elseif textedit and textedit:match("(.*)(#)(.*)") then
if redis:get(bot_id.."Olifay:Lock:hashtak"..msg.chat_id_) then
Delete_Message(result.chat_id_,{[0] = data.message_id_}) 
return false
end  
elseif textedit and textedit:match("#") then
...
elseif textedit and textedit:match("/") then
if redis:get(bot_id.."Olifay:Lock:Cmd"..msg.chat_id_) then
Delete_Message(result.chat_id_,{[0] = data.message_id_}) 
return false
end 
elseif textedit and textedit:match("(.*)(/)(.*)") then
...
local Text_Filter = redis:get(bot_id.."Olifay:Filter:Reply2"..textedit..result.chat_id_)   
if Text_Filter then    
Delete_Message(result.chat_id_, {[0] = data.message_id_})     
...
local Get_Msg_Pin = redis:get(bot_id..'Olifay:Msg:Pin:Chat'..msg.chat_id_)
if Get_Msg_Pin ~= nil then
if text == Get_Msg_Pin then
tdcli_function ({ID = "PinChannelMessage",channel_id_ = msg.chat_id_:gsub('-100',''),message_id_ = msg.id_,disable_notification_ = 0},function(arg,d) if d.ID == 'Ok' then;redis:del(bot_id..'Olifay:Msg:Pin:Chat'..msg.chat_id_);end;end,nil)   
elseif (msg.content_.sticker_) then 
if Get_Msg_Pin == msg.content_.sticker_.sticker_.persistent_id_ then
tdcli_function ({ID = "PinChannelMessage",channel_id_ = msg.chat_id_:gsub('-100',''),message_id_ = msg.id_,disable_notification_ = 0},function(arg,d) redis:del(bot_id..'Olifay:Msg:Pin:Chat'..msg.chat_id_) end,nil)   
end
end
if (msg.content_.animation_) then 
...
redis:srem(bot_id..'Olifay:Chek:Groups',v)  
end
if data and data.type_ and data.type_.channel_ and data.type_.channel_.status_ and data.type_.channel_.status_.ID == "ChatMemberStatusLeft" then
redis:srem(bot_id..'Olifay:Chek:Groups',v)  
end
...
redis:sadd(bot_id..'Olifay:Chek:Groups',v)  
end 
end,nil)
end
end
